...
},setCookie:function(a,b,c){Util.setCookie(b,a,10*365*24*60*60,location.hostname)
...
var l=d.vars.namespace,f=window.navigator&&window.navigator.msPointerEnabled&&window.MSGesture,m=(("ontouchstart" in window)||f||window.DocumentTouch&&document instanceof DocumentTouch)&&d.vars.touch,e="click touchend MSPointerUp",c="",q,k=d.vars.direction==="vertical",n=d.vars.reverse,p=(d.vars.itemWidth>0),j=d.vars.animation==="fade",o=d.vars.asNavFor!=="",h={};
...
controlRole=(d.vars.useAriaAttributes===true)?' role="button" ':"";
...
x=(d.vars.controlNav==="thumbnails")?'<img src="'+s.attr("data-thumb")+'"'+t+controlRole+"/>":'<a href="#"'+controlRole+">"+u+"</a>";
...
}},directionNav:{setup:function(){var s=a('<ul class="'+l+'direction-nav"><li class="'+l+'nav-prev"><a role="button" class="'+l+'prev" href="#">'+d.vars.prevText+'</a></li><li class="'+l+'nav-next"><a role="button" class="'+l+'next" href="#">'+d.vars.nextText+"</a></li></ul>");
...
if(d.pagingCount===1){d.directionNav.addClass(s)
}else{if(!d.vars.animationLoop){if(d.animatingTo===0){d.directionNav.removeClass(s).filter("."+l+"prev").addClass(s)
}else{if(d.animatingTo===d.last){d.directionNav.removeClass(s).filter("."+l+"next").addClass(s)
}else{d.directionNav.removeClass(s)
}}}else{d.directionNav.removeClass(s)
}}}},pausePlay:{setup:function(){controlRole=(d.vars.useAriaAttributes===true)?' role="button" ':"";
var s=a('<div class="'+l+'pauseplay"><a href="#" + '+controlRole+"></a></div>");
...
(s)?t.animate({height:d.slides.eq(d.animatingTo).innerHeight()},s).css("overflow","visible"):t.innerHeight(d.slides.eq(d.animatingTo).innerHeight())
...
}}else{if(!m){d.slides.eq(d.currentSlide).css({zIndex:1,display:"none"}).animate({opacity:0},d.vars.animationSpeed,d.vars.easing);
d.slides.eq(A).css({zIndex:2,display:"block"}).animate({opacity:1},d.vars.animationSpeed,d.vars.easing,d.wrapup)
}else{d.slides.eq(d.currentSlide).css({opacity:0,zIndex:1,display:"none"});
d.slides.eq(A).css({opacity:1,zIndex:2,display:"block"});
d.wrapup(z)
...
d.slideAttributes=function(s){l=d.vars.namespace;
if(d.vars.useAriaAttributes==true){d.slides.removeClass(l+"active-slide").attr("aria-hidden","true").eq(s).addClass(l+"active-slide").removeAttr("aria-hidden");
if(d.vars.ariaPolite){d.slides.find(l+"caption").removeAttr("aria-live");
if(d.playing===false){d.slides.eq(s).find("."+l+"caption").attr("aria-live","polite")
}}}else{d.slides.removeClass(l+"active-slide").eq(s).addClass(l+"active-slide")
}};
...
if(t==="init"){if(!m){if(d.vars.fadeFirstSlide==false){d.slides.css({opacity:0,display:"none",zIndex:1}).eq(d.currentSlide).css({zIndex:2,display:"block"}).css({opacity:1})
}else{d.slides.css({opacity:0,display:"none",zIndex:1}).eq(d.currentSlide).css({zIndex:2,display:"block"}).animate({opacity:1},d.vars.animationSpeed,d.vars.easing)
}}else{d.slides.css({opacity:0,display:"none",webkitTransition:"opacity "+d.vars.animationSpeed/1000+"s ease",zIndex:1}).eq(d.currentSlide).css({opacity:1,zIndex:2,display:"block"})
}}if(d.vars.smoothHeight){h.smoothHeight()
}}if(!p){d.slideAttributes(d.currentSlide)
}d.vars.init(d)
};
...
a.flexslider.defaults={namespace:"flex-",selector:".slides > li",animation:"fade",easing:"swing",direction:"horizontal",reverse:false,animationLoop:true,smoothHeight:false,startAt:0,slideshow:true,slideshowSpeed:7000,animationSpeed:600,initDelay:0,randomize:false,fadeFirstSlide:true,thumbCaptions:false,pauseOnAction:true,pauseOnHover:false,pauseInvisible:true,useCSS:true,touch:true,video:false,controlNav:true,directionNav:true,prevText:"Previous",nextText:"Next",keyboard:true,multipleKeyboard:false,mousewheel:false,pausePlay:false,pauseText:"Pause",playText:"Play",controlsContainer:"",manualControls:"",customDirectionNav:"",sync:"",asNavFor:"",itemWidth:0,itemMargin:0,minItems:1,maxItems:0,move:0,allowOneSlide:true,useAriaAttributes:true,ariaPolite:true,start:function(){},before:function(){},after:function(){},end:function(){},added:function(){},removed:function(){},init:function(){}};
...
}var addeventatc=function(){var ag,Q,ak,Y,X,ah=!1,ai=1,ac=!1,aa=!0,U=!1,R=!1,W=!1,O=1,M="",Z=!0,ae=!0,ad=!0,af=!0,aj=!0,K=!0,J="Apple Calendar",ab="Google <em>(online)</em>",G="Outlook",al="Outlook.com <em>(online)</em>",j="Yahoo <em>(online)</em>",F="Facebook Event",P=null,B=null,S=null,H=null,q=null,V=null,D=!1;
return{initialize:function(){if(!ah){ah=!0;
try{addeventasync()
}catch(a){}"https:","addevent.com","https://addevent.com",a="undefined"!=typeof SVGRect?"https://www.addevent.com/gfx/icon-calendar-t1.svg":"https://www.addevent.com/gfx/icon-calendar-t5.png",addeventatc.trycss(),addeventatc.generate()
}},generate:function(){for(var b=document.getElementsByTagName("*"),a=0;
a<b.length;
a+=1){addeventatc.hasclass(b[a],"addeventatc")&&function(){var c="addeventatc"+ai;
b[a].id=c,b[a].title="",b[a].style.visibility="visible",b[a].setAttribute("aria-haspopup","true"),b[a].setAttribute("aria-expanded","false"),b[a].setAttribute("tabindex","0"),ac?(b[a].onclick=function(){return !1
},b[a].onmouseover=function(){clearTimeout(Y),addeventatc.toogle(this,{type:"mouseover",id:c})
},b[a].onmouseout=function(){Y=setTimeout(function(){addeventatc.mouseout(this,{type:"mouseout",id:c})
},100)
}):(b[a].onclick=function(){return addeventatc.toogle(this,{type:"click",id:c}),!1
},b[a].onmouseover=function(){},b[a].onmouseout=function(){}),b[a].onkeydown=function(l){var d=l.which||l.keyCode;
"13"!=d&&"32"!=d&&"27"!=d&&"38"!=d&&"40"!=d||l.preventDefault(),"13"!=d&&"32"!=d||(addeventatc.keyboardclick(this,{type:"click",id:c}),addeventatc.toogle(this,{type:"click",id:c,keynav:!0})),"27"==d&&addeventatc.hideandreset(),"38"==d&&addeventatc.keyboard(this,{type:"keyboard",id:c,key:"up"}),"40"==d&&addeventatc.keyboard(this,{type:"keyboard",id:c,key:"down"}),D=!0
},b[a].onblur=function(){D&&setTimeout(function(){addeventatc.hideandreset()
},300)
};
var k=b[a];
if("none"!=b[a].getAttribute("data-styling")&&"inline-buttons"!=b[a].getAttribute("data-render")||(aa=!1),aa){var h=document.createElement("span");
h.className="addeventatc_icon",k.appendChild(h)
}ai++,U=!0;
for(var f=b[a].getElementsByTagName("*"),g=0;
g<f.length;
g+=1){addeventatc.hasclass(f[g],"atc_node")||(""!=f[g].className?f[g].className+=" atc_node":f[g].className+="atc_node")
}if("inline-buttons"==b[a].getAttribute("data-render")){b[a].onclick=function(){},addeventatc.toogle(b[a],{type:"render",id:c}),b[a].setAttribute("aria-expanded","true"),b[a].removeAttribute("tabindex"),b[a].onkeydown=function(){},b[a].blur=function(){};
var e=document.getElementById(c+"-drop");
if(e){e.setAttribute("aria-hidden","false");
for(f=e.getElementsByTagName("SPAN"),g=0;
g<f.length;
g+=1){f[g];
f[g].tabIndex="0",f[g].onkeydown=function(l){var d=l.which||l.keyCode;
"13"!=d&&"32"!=d||l.target.click()
}
}}}}()
}O=addeventatc.topzindex(),aa?addeventatc.applycss():(addeventatc.removeelement($d("ate_css")),addeventatc.removeelement($d("ate_tmp_css")),addeventatc.helpercss()),U&&!R&&(R=!0,addeventatc.track({typ:"exposure",cal:""}))
},toogle:function(m,g){var f,p,t,k=!1,h="";
if(p=$d(f=m.id)){t=p.getAttribute("data-direct");
var a=p.getAttribute("data-intel"),x=p.getAttribute("data-intel-apple");
if("ios"==addeventatc.agent()&&"click"==g.type&&"true"!==x&&"false"!==a&&(t="appleical",p.setAttribute("data-intel-apple","true")),"outlook"==t||"google"==t||"yahoo"==t||"hotmail"==t||"outlookcom"==t||"appleical"==t||"apple"==t||"facebook"==t){"click"==g.type&&(addeventatc.click({button:f,service:t,id:g.id}),null!=P&&addeventatc.trigger("button_click",{id:f}))
}else{if("mouseover"==g.type&&ak!=p&&(W=!1),"click"==g.type||"render"==g.type||"mouseover"==g.type&&!W){"mouseover"==g.type&&(W=!0,null!=B&&addeventatc.trigger("button_mouseover",{id:f})),k=addeventatc.getburl({id:f,facebook:!0}),""==M&&(M="appleical,google,outlook,outlookcom,yahoo,facebook");
for(var w=(M=(M+=",").replace(/ /gi,"")).split(","),b=0;
b<w.length;
b+=1){(Z&&"ical"==w[b]||Z&&"appleical"==w[b])&&(h+='<span class="ateappleical" id="'+f+'-appleical" role="menuitem">'+J+"</span>"),ae&&"google"==w[b]&&(h+='<span class="ategoogle" id="'+f+'-google" role="menuitem">'+ab+"</span>"),ad&&"outlook"==w[b]&&(h+='<span class="ateoutlook" id="'+f+'-outlook" role="menuitem">'+G+"</span>"),(af&&"hotmail"==w[b]||af&&"outlookcom"==w[b])&&(h+='<span class="ateoutlookcom" id="'+f+'-outlookcom" role="menuitem">'+al+"</span>"),aj&&"yahoo"==w[b]&&(h+='<span class="ateyahoo" id="'+f+'-yahoo" role="menuitem">'+j+"</span>"),k&&"facebook"==w[b]&&K&&"facebook"==w[b]&&(h+='<span class="atefacebook" id="'+f+'-facebook" role="menuitem">'+F+"</span>")
}if(addeventatc.getlicense(Q)||(h+='<em class="copyx"><em class="brx"></em><em class="frs"><a href="https://www.addevent.com" title="" tabindex="-1" id="'+f+'-home">AddEvent.com</a></em></em>'),!$d(f+"-drop")){var v=document.createElement("span");
v.id=f+"-drop",v.className="addeventatc_dropdown",v.setAttribute("aria-hidden","true"),v.setAttribute("aria-labelledby",f),v.innerHTML=h,p.appendChild(v)
}$d(f+"-appleical")&&($d(f+"-appleical").onclick=function(){addeventatc.click({button:f,service:"appleical",id:g.id})
}),$d(f+"-google")&&($d(f+"-google").onclick=function(){addeventatc.click({button:f,service:"google",id:g.id})
}),$d(f+"-outlook")&&($d(f+"-outlook").onclick=function(){addeventatc.click({button:f,service:"outlook",id:g.id})
}),$d(f+"-outlookcom")&&($d(f+"-outlookcom").onclick=function(){addeventatc.click({button:f,service:"outlookcom",id:g.id})
}),$d(f+"-yahoo")&&($d(f+"-yahoo").onclick=function(){addeventatc.click({button:f,service:"yahoo",id:g.id})
}),$d(f+"-facebook")&&($d(f+"-facebook").onclick=function(){addeventatc.click({button:f,service:"facebook",id:g.id})
}),$d(f+"-home")&&($d(f+"-home").onclick=function(){addeventatc.click({button:f,service:"home",id:g.id})
}),addeventatc.show(f,g)
}}return ak=p,!1
}},buildIcs:function(x){var d=jQuery("span.start").text();
var b=jQuery("span.end").text();
var s=jQuery("span.timezone").text();
var n=jQuery("span.description").text();
var f=jQuery("span.location").text();
var w=jQuery("span.title").text();
var m=jQuery("span.date_format").text();
if(moment){var t=moment.tz(jQuery("span.start").text(),jQuery("span.date_format").text(),jQuery("span.timezone").text()).utc().format("YYYYMMDD[T]HHmmss[Z]");
var o=moment.tz(jQuery("span.end").text(),jQuery("span.date_format").text(),jQuery("span.timezone").text()).utc().format("YYYYMMDD[T]HHmmss[Z]");
if(x=="appleical"||x=="outlook"){var l="data:text/calendar;charset=utf8,BEGIN:VCALENDAR%0AVERSION:2.0%0ABEGIN:VEVENT%0ADTSTART:"+t+"%0ADTEND:"+o+"%0ASUMMARY:"+w+"%0ADESCRIPTION:"+n+"%0ALOCATION:"+f+"%0AEND:VEVENT%0AEND:VCALENDAR";
var v="";
return l+v
}else{if(x=="google"){var p="https://calendar.google.com/calendar/r/eventedit?text=";
var e=p+w+"&dates="+t+"/"+o+"&details="+n+"&in_loc="+f+"&sprop=&sprop=name:";
return e
}else{if(x=="yahoo"){var g=moment(t,"YYYYMMDD[T]HHmmss[Z]");
var c=moment(o,"YYYYMMDD[T]HHmmss[Z]");
var a=c.diff(g,"minutes",true);
var u=moment.duration(a,"minutes");
var k="";
var r="";
var h="";
if(u.days()>0){k=Number(k)+(u.days()*24)*100
}k=Number(k)+(u.hours()*100);
r=k;
if(u.minutes()>0){k=Number(k)+u.minutes();
h=k
}if(r.toString()==0){k="00"+k
}else{if(r.toString().length==3){k="0"+k
}}if(h.toString().length==1){k="0"+k
}var p="https://calendar.yahoo.com/?v=60&view=d&type=20&title=";
var e=p+w+"&st="+t+"&dur="+k+"&desc="+n+"&in_loc="+f;
return e
}}}}},click:function(c){if(c.service=="appleical"||c.service=="outlook"){var b=addeventatc.buildIcs(c.service);
window.open(b,"_self")
}else{if(c.service=="google"||c.service=="yahoo"){var a=addeventatc.buildIcs(c.service);
window.open(encodeURI(a),"_blank")
}else{}}},mouseout:function(b,a){W=!1,addeventatc.hideandreset(),null!=S&&addeventatc.trigger("button_mouseout",{id:a.id})
},show:function(av,E){var az=$d(av),am=$d(av+"-drop");
if(az&&am){if("block"==addeventatc.getstyle(am,"display")){addeventatc.hideandreset()
}else{addeventatc.hideandreset(!0),am.style.display="block",az.style.outline="0",az.style.zIndex=O+1,az.className=az.className+" addeventatc-selected",az.className=az.className.replace(/\s+/g," "),az.setAttribute("aria-expanded","true"),am.setAttribute("aria-hidden","false"),E.keynav&&addeventatc.keyboard(this,{type:"keyboard",id:av,key:"down"});
var T=az.getAttribute("data-dropdown-x"),aw=az.getAttribute("data-dropdown-y"),ax="auto",aq="auto";
null!=T&&(aq=T),null!=aw&&(ax=aw),am.style.left="0px",am.style.top="0px",am.style.display="block";
var ao=parseInt(az.offsetHeight),L=parseInt(az.offsetWidth),I=parseInt(am.offsetHeight),N=parseInt(am.offsetWidth),C=addeventatc.viewport(),an=parseInt(C.w),at=parseInt(C.h),ar=parseInt(C.x),au=parseInt(C.y),ay=addeventatc.elementposition(am),A=parseInt(ay.x),z=parseInt(ay.y)+I,ap=at+au,u=0,aA=0;
"down"==ax&&"left"==aq?(u="0px",aA=ao+"px"):"up"==ax&&"left"==aq?(u="0px",aA=-I+"px"):"down"==ax&&"right"==aq?(u=-(N-L)+"px",aA=ao+"px"):"up"==ax&&"right"==aq?(u=-(N-L)+"px",aA=-I+"px"):"auto"==ax&&"left"==aq?(u="0px",aA=z>ap?-I+"px":ao+"px"):"auto"==ax&&"right"==aq?(u=-(N-L)+"px",aA=z>ap?-I+"px":ao+"px"):(aA=z>ap?-I+"px":ao+"px",u=A+N>an+ar?-(N-L)+"px":"0px"),am.style.left=u,am.style.top=aA,"click"==E.type&&null!=P&&addeventatc.trigger("button_click",{id:av}),null!=H&&addeventatc.trigger("button_dropdown_show",{id:av})
}}},hide:function(b){var a=!1;
("string"==typeof b&&""!==b||b instanceof String&&""!==b)&&(b.indexOf("addeventatc")>-1||b.indexOf("atc_node")>-1)&&(a=!0),a||addeventatc.hideandreset()
},hideandreset:function(k){for(var s="",r=document.getElementsByTagName("*"),f=0;
f<r.length;
f+=1){if(addeventatc.hasclass(r[f],"addeventatc")){r[f].className=r[f].className.replace(/addeventatc-selected/gi,""),r[f].className=r[f].className.replace(/\s+$/,""),r[f].style.zIndex="auto",r[f].style.outline="";
var b=$d(r[f].id+"-drop");
if(b){var m=addeventatc.getstyle(b,"display");
"block"==m&&(s=r[f].id),b.style.display="none","block"!==(m=addeventatc.getstyle(b,"display"))&&(r[f].setAttribute("aria-expanded","false"),b.setAttribute("aria-hidden","true"));
for(var p=b.getElementsByTagName("SPAN"),h=0;
h<p.length;
h+=1){var g=new RegExp("(\\s|^)drop_markup(\\s|$)");
p[h].className=p[h].className.replace(g," "),p[h].className=p[h].className.replace(/\s+$/,"")
}}}}k||null!=q&&addeventatc.trigger("button_dropdown_hide",{id:s})
},getburl:function(f){var c=$d(f.id),b="",k=!1;
if(c){for(var h=c.getElementsByTagName("*"),g=0;
g<h.length;
g+=1){(addeventatc.hasclass(h[g],"_start")||addeventatc.hasclass(h[g],"start"))&&(b+="&dstart="+encodeURIComponent(h[g].innerHTML)),(addeventatc.hasclass(h[g],"_end")||addeventatc.hasclass(h[g],"end"))&&(b+="&dend="+encodeURIComponent(h[g].innerHTML)),(addeventatc.hasclass(h[g],"_zonecode")||addeventatc.hasclass(h[g],"zonecode"))&&(b+="&dzone="+encodeURIComponent(h[g].innerHTML)),(addeventatc.hasclass(h[g],"_timezone")||addeventatc.hasclass(h[g],"timezone"))&&(b+="&dtime="+encodeURIComponent(h[g].innerHTML)),(addeventatc.hasclass(h[g],"_summary")||addeventatc.hasclass(h[g],"summary")||addeventatc.hasclass(h[g],"title"))&&(b+="&dsum="+encodeURIComponent(h[g].innerHTML)),(addeventatc.hasclass(h[g],"_description")||addeventatc.hasclass(h[g],"description"))&&(b+="&ddesc="+encodeURIComponent(h[g].innerHTML)),(addeventatc.hasclass(h[g],"_location")||addeventatc.hasclass(h[g],"location"))&&(b+="&dloca="+encodeURIComponent(h[g].innerHTML)),(addeventatc.hasclass(h[g],"_organizer")||addeventatc.hasclass(h[g],"organizer"))&&(b+="&dorga="+encodeURIComponent(h[g].innerHTML)),(addeventatc.hasclass(h[g],"_organizer_email")||addeventatc.hasclass(h[g],"organizer_email"))&&(b+="&dorgaem="+encodeURIComponent(h[g].innerHTML)),(addeventatc.hasclass(h[g],"_attendees")||addeventatc.hasclass(h[g],"attendees"))&&(b+="&datte="+encodeURIComponent(h[g].innerHTML)),(addeventatc.hasclass(h[g],"_all_day_event")||addeventatc.hasclass(h[g],"all_day_event"))&&(b+="&dallday="+encodeURIComponent(h[g].innerHTML)),(addeventatc.hasclass(h[g],"_date_format")||addeventatc.hasclass(h[g],"date_format"))&&(b+="&dateformat="+encodeURIComponent(h[g].innerHTML)),(addeventatc.hasclass(h[g],"_alarm_reminder")||addeventatc.hasclass(h[g],"alarm_reminder"))&&(b+="&alarm="+encodeURIComponent(h[g].innerHTML)),(addeventatc.hasclass(h[g],"_recurring")||addeventatc.hasclass(h[g],"recurring"))&&(b+="&drule="+encodeURIComponent(h[g].innerHTML)),(addeventatc.hasclass(h[g],"_facebook_event")||addeventatc.hasclass(h[g],"facebook_event"))&&(b+="&fbevent="+encodeURIComponent(h[g].innerHTML),k=!0),(addeventatc.hasclass(h[g],"_client")||addeventatc.hasclass(h[g],"client"))&&(b+="&client="+encodeURIComponent(h[g].innerHTML)),(addeventatc.hasclass(h[g],"_calname")||addeventatc.hasclass(h[g],"calname"))&&(b+="&calname="+encodeURIComponent(h[g].innerHTML)),(addeventatc.hasclass(h[g],"_uid")||addeventatc.hasclass(h[g],"uid"))&&(b+="&uid="+encodeURIComponent(h[g].innerHTML)),(addeventatc.hasclass(h[g],"_status")||addeventatc.hasclass(h[g],"status"))&&(b+="&status="+encodeURIComponent(h[g].innerHTML)),(addeventatc.hasclass(h[g],"_method")||addeventatc.hasclass(h[g],"method"))&&(b+="&method="+encodeURIComponent(h[g].innerHTML))
}"false"==c.getAttribute("data-google-api")&&(b+="&gapi=false")
}return f.facebook&&(b=k),b
},trycss:function(){if(!$d("ate_tmp_css")){try{var b="";
b=".addeventatc {visibility:hidden;}",b+=".addeventatc .data {display:none!important;}",b+=".addeventatc .start, .addeventatc .end, .addeventatc .timezone, .addeventatc .title, .addeventatc .description, .addeventatc .location, .addeventatc .organizer, .addeventatc .organizer_email, .addeventatc .facebook_event, .addeventatc .all_day_event, .addeventatc .date_format, .addeventatc .alarm_reminder, .addeventatc .recurring, .addeventatc .attendees, .addeventatc .client, .addeventatc .calname, .addeventatc .uid, .addeventatc .status, .addeventatc .method {display:none!important;}",aa&&(b+=".addeventatc {background-image:url(https://www.addevent.com/gfx/icon-calendar-t5.png), url(https://www.addevent.com/gfx/icon-calendar-t1.svg), url(https://www.addevent.com/gfx/icon-apple-t1.svg), url(https://www.addevent.com/gfx/icon-facebook-t1.svg), url(https://www.addevent.com/gfx/icon-google-t1.svg), url(https://www.addevent.com/gfx/icon-outlook-t1.svg), url(https://www.addevent.com/gfx/icon-yahoo-t1.svg);background-position:-9999px -9999px;background-repeat:no-repeat;}");
var a=document.createElement("style");
a.type="text/css",a.id="ate_tmp_css",a.styleSheet?a.styleSheet.cssText=b:a.appendChild(document.createTextNode(b)),document.getElementsByTagName("head")[0].appendChild(a)
}catch(b){}addeventatc.track({typ:"jsinit",cal:""})
}},applycss:function(){if(!$d("ate_css")){var b="";
b+='.addeventatc {display:inline-block;*display:inline;zoom:1;position:relative;z-index:1;font-family:Roboto,"Helvetica Neue",Helvetica,Optima,Segoe,"Segoe UI",Candara,Calibri,Arial,sans-serif;color:#000!important;font-weight:300;line-height:100%!important;background-color:#fff;border:1px solid;border-color:#e5e6e9 #dfe0e4 #d0d1d5;font-size:15px;text-decoration:none;padding:13px 12px 12px 43px;-webkit-border-radius:3px;border-radius:3px;cursor:pointer;-webkit-font-smoothing:antialiased!important;text-shadow:1px 1px 1px rgba(0,0,0,0.004);-webkit-touch-callout:none;-webkit-user-select:none;-khtml-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none;-webkit-tap-highlight-color:rgba(0,0,0,0);background-image:url(https://www.addevent.com/gfx/icon-calendar-t5.png), url(https://www.addevent.com/gfx/icon-calendar-t1.svg), url(https://www.addevent.com/gfx/icon-apple-t1.svg), url(https://www.addevent.com/gfx/icon-facebook-t1.svg), url(https://www.addevent.com/gfx/icon-google-t1.svg), url(https://www.addevent.com/gfx/icon-outlook-t1.svg), url(https://www.addevent.com/gfx/icon-yahoo-t1.svg);background-position:-9999px -9999px;background-repeat:no-repeat;}',b+=".addeventatc:hover {border:1px solid #aab9d4;color:#000;font-size:15px;text-decoration:none;}",b+=".addeventatc:focus {outline:none;border:1px solid #aab9d4;}",b+=".addeventatc:active {top:1px;}",b+=".addeventatc-selected {background-color:#f9f9f9;}",b+=".addeventatc .addeventatc_icon {width:18px;height:18px;position:absolute;z-index:1;left:12px;top:10px;background:url("+ag+") no-repeat;background-size:18px 18px;}",b+=".addeventatc .start, .addeventatc .end, .addeventatc .timezone, .addeventatc .title, .addeventatc .description, .addeventatc .location, .addeventatc .organizer, .addeventatc .organizer_email, .addeventatc .facebook_event, .addeventatc .all_day_event, .addeventatc .date_format, .addeventatc .alarm_reminder, .addeventatc .recurring, .addeventatc .attendees, .addeventatc .client, .addeventatc .calname, .addeventatc .uid, .addeventatc .status, .addeventatc .method {display:none!important;}",b+=".addeventatc .data {display:none!important;}",b+=".addeventatc br {display:none;}",addeventatc.getlicense(Q)?b+=".addeventatc_dropdown {width:200px;position:absolute;z-index:99999;padding:6px 0px 6px 0px;background:#fff;text-align:left;display:none;margin-top:-2px;margin-left:-1px;border-top:1px solid #c8c8c8;border-right:1px solid #bebebe;border-bottom:1px solid #a8a8a8;border-left:1px solid #bebebe;-moz-border-radius:2px;-webkit-border-radius:2px;-webkit-box-shadow:1px 3px 6px rgba(0,0,0,0.15);-moz-box-shadow:1px 3px 6px rgba(0,0,0,0.15);box-shadow:1px 3px 6px rgba(0,0,0,0.15);}":b+=".addeventatc_dropdown {width:200px;position:absolute;z-index:99999;padding:6px 0px 0px 0px;background:#fff;text-align:left;display:none;margin-top:-2px;margin-left:-1px;border-top:1px solid #c8c8c8;border-right:1px solid #bebebe;border-bottom:1px solid #a8a8a8;border-left:1px solid #bebebe;-moz-border-radius:2px;-webkit-border-radius:2px;-webkit-box-shadow:1px 3px 6px rgba(0,0,0,0.15);-moz-box-shadow:1px 3px 6px rgba(0,0,0,0.15);box-shadow:1px 3px 6px rgba(0,0,0,0.15);}",b+=".addeventatc_dropdown span {display:block;line-height:100%;background:#fff;text-decoration:none;font-size:14px;color:#333;padding:9px 10px 9px 40px;}",b+=".addeventatc_dropdown span:hover {background-color:#f4f4f4;color:#000;text-decoration:none;font-size:14px;}",b+=".addeventatc_dropdown .drop_markup {background-color:#f4f4f4;color:#000;text-decoration:none;font-size:14px;}",b+=".addeventatc_dropdown .copyx {height:21px;display:block;position:relative;cursor:default;}",b+=".addeventatc_dropdown .brx {height:1px;overflow:hidden;background:#e0e0e0;position:absolute;z-index:100;left:10px;right:10px;top:9px;}",b+=".addeventatc_dropdown .frs {position:absolute;top:5px;cursor:pointer;right:10px;font-style:normal!important;font-weight:normal!important;text-align:right;z-index:101;line-height:9px!important;background:#fff;text-decoration:none;font-size:9px!important;color:#cacaca!important;}",b+=".addeventatc_dropdown .frs a {margin:0!important;padding:0!important;font-style:normal!important;font-weight:normal!important;line-height:9px!important;background-color:#fff!important;text-decoration:none;font-size:9px!important;color:#cacaca!important;padding-left:10px!important;display:inline-block;}",b+=".addeventatc_dropdown .frs a:hover {color:#999!important;}",b+=".addeventatc_dropdown .ateappleical {background-image:url(https://www.addevent.com/gfx/icon-apple-t1.svg);background-repeat:no-repeat;background-position:13px 50%;background-size:14px 100%;}",b+=".addeventatc_dropdown .ategoogle {background-image:url(https://www.addevent.com/gfx/icon-google-t1.svg);background-repeat:no-repeat;background-position:12px 50%;background-size:16px 100%;}",b+=".addeventatc_dropdown .ateoutlook {background-image:url(https://www.addevent.com/gfx/icon-outlook-t1.svg);background-repeat:no-repeat;background-position:12px 50%;background-size:16px auto;}",b+=".addeventatc_dropdown .ateoutlookcom {background-image:url(https://www.addevent.com/gfx/icon-outlook-t1.svg);background-repeat:no-repeat;background-position:12px 50%;background-size:16px auto;}",b+=".addeventatc_dropdown .ateyahoo {background-image:url(https://www.addevent.com/gfx/icon-yahoo-t1.svg);background-repeat:no-repeat;background-position:12px 50%;background-size:16px auto;}",b+=".addeventatc_dropdown .atefacebook {background-image:url(https://www.addevent.com/gfx/icon-facebook-t1.svg);background-repeat:no-repeat;background-position:12px 50%;background-size:16px auto;}",b+=".addeventatc_dropdown em {font-size:12px!important;color:#999!important;}";
var c=document.createElement("style");
c.type="text/css",c.id="ate_css",c.styleSheet?c.styleSheet.cssText=b:c.appendChild(document.createTextNode(b)),document.getElementsByTagName("head")[0].appendChild(c),addeventatc.removeelement($d("ate_tmp_css"))
}},helpercss:function(){if(!$d("ate_helper_css")){var b="";
b+=".addeventatc_dropdown .drop_markup {background-color:#f4f4f4;}",b+=".addeventatc_dropdown .frs a {margin:0!important;padding:0!important;font-style:normal!important;font-weight:normal!important;line-height:110%!important;background-color:#fff!important;text-decoration:none;font-size:9px!important;color:#cacaca!important;display:inline-block;}",b+=".addeventatc_dropdown .frs a:hover {color:#999!important;}",b+=".addeventatc .start, .addeventatc .end, .addeventatc .timezone, .addeventatc .title, .addeventatc .description, .addeventatc .location, .addeventatc .organizer, .addeventatc .organizer_email, .addeventatc .facebook_event, .addeventatc .all_day_event, .addeventatc .date_format, .addeventatc .alarm_reminder, .addeventatc .recurring, .addeventatc .attendees, .addeventatc .client, .addeventatc .calname, .addeventatc .uid, .addeventatc .status, .addeventatc .method {display:none!important;}";
var a=document.createElement("style");
a.type="text/css",a.id="ate_helper_css",a.styleSheet?a.styleSheet.cssText=b:a.appendChild(document.createTextNode(b)),document.getElementsByTagName("head")[0].appendChild(a)
}},removeelement:function(a){try{return !!(hdx=a)&&hdx.parentNode.removeChild(hdx)
}catch(a){}},topzindex:function(){for(var d=1,c=document.getElementsByTagName("*"),b=0;
b<c.length;
b+=1){if(addeventatc.hasclass(c[b],"addeventatc")||addeventatc.hasclass(c[b],"addeventstc")){var f=addeventatc.getstyle(c[b],"z-index");
!isNaN(parseFloat(f))&&isFinite(f)&&(f=parseInt(f))>d&&(d=f)
}}return d
},viewport:function(){var d=0,c=0,b=0,f=0;
return"number"==typeof window.innerWidth?(d=window.innerWidth,c=window.innerHeight):document.documentElement&&(document.documentElement.clientWidth||document.documentElement.clientHeight)?(d=document.documentElement.clientWidth,c=document.documentElement.clientHeight):document.body&&(document.body.clientWidth||document.body.clientHeight)&&(d=document.body.clientWidth,c=document.body.clientHeight),document.all?(f=document.documentElement.scrollLeft?document.documentElement.scrollLeft:document.body.scrollLeft,b=document.documentElement.scrollTop?document.documentElement.scrollTop:document.body.scrollTop):(f=window.pageXOffset,b=window.pageYOffset),{w:d,h:c,x:f,y:b}
},elementposition:function(d){var c=0,b=0;
if(d.offsetParent){for(c=d.offsetLeft,b=d.offsetTop;
d=d.offsetParent;
){c+=d.offsetLeft,b+=d.offsetTop
}}return{x:c,y:b}
},getstyle:function(d,c){var b,f=d;
return f.currentStyle?b=f.currentStyle[c]:window.getComputedStyle&&(b=document.defaultView.getComputedStyle(f,null).getPropertyValue(c)),b
},getlicense:function(f){var c=location.origin,b=!1;
if(void 0===location.origin&&(c=location.protocol+"//"+location.host),f){var k=f.substring(0,1),h=f.substring(9,10),g=f.substring(17,18);
"a"==k&&"z"==h&&"m"==g&&(b=!0)
}return(-1==c.indexOf("addevent.com")&&"aao8iuet5zp9iqw5sm9z"==f||-1==c.indexOf("addevent.to")&&"aao8iuet5zp9iqw5sm9z"==f||-1==c.indexOf("addevent.com")&&"aao8iuet5zp9iqw5sm9z"==f)&&(b=!0),b
},refresh:function(){for(var f=document.getElementsByTagName("*"),c=[],b=0;
b<f.length;
b+=1){if(addeventatc.hasclass(f[b],"addeventatc")){f[b].className=f[b].className.replace(/addeventatc-selected/gi,""),f[b].id="";
for(var k=f[b].getElementsByTagName("*"),h=0;
h<k.length;
h+=1){(addeventatc.hasclass(k[h],"addeventatc_icon")||addeventatc.hasclass(k[h],"addeventatc_dropdown"))&&c.push(k[h])
}}}for(var g=0;
g<c.length;
g+=1){addeventatc.removeelement(c[g])
}addeventatc.removeelement($d("ate_css")),ai=1,R=!1,addeventatc.generate()
},hasclass:function(b,a){return new RegExp("(\\s|^)"+a+"(\\s|$)").test(b.className)
},eclick:function(d){var c=document.getElementById(d);
if(c.click){c.click()
}else{if(document.createEvent){var b=document.createEvent("MouseEvents");
b.initEvent("click",!0,!0),c.dispatchEvent(b)
}}},track:function(a){new Image,(new Date).getTime(),encodeURIComponent(window.location.origin)
},getguid:function(){for(var h="addevent_track_cookie=",g="",b=document.cookie.split(";"),p=0;
p<b.length;
p++){for(var l=b[p];
" "==l.charAt(0);
){l=l.substring(1,l.length)
}0==l.indexOf(h)&&(g=l.substring(h.length,l.length))
}if(""==g){var k=(addeventatc.s4()+addeventatc.s4()+"-"+addeventatc.s4()+"-4"+addeventatc.s4().substr(0,3)+"-"+addeventatc.s4()+"-"+addeventatc.s4()+addeventatc.s4()+addeventatc.s4()).toLowerCase(),m=new Date;
m.setTime(m.getTime()+31536000000);
var f="expires="+m.toUTCString();
document.cookie="addevent_track_cookie="+k+"; "+f,g=k
}return g
},s4:function(){return(65536*(1+Math.random())|0).toString(16).substring(1)
},documentclick:function(a){a=(a=a||window.event).target||a.srcElement,ate_touch_capable?(clearTimeout(X),X=setTimeout(function(){addeventatc.hide(a.className)
},500)):addeventatc.hide(a.className)
},trigger:function(b,a){if("button_click"==b){try{P(a)
}catch(b){}}if("button_mouseover"==b){try{B(a)
}catch(b){}}if("button_mouseout"==b){try{S(a)
}catch(b){}}if("button_dropdown_show"==b){try{H(a)
}catch(b){}}if("button_dropdown_hide"==b){try{q(a)
}catch(b){}}if("button_dropdown_click"==b){try{V(a)
}catch(b){}}},register:function(b,a){"button-click"==b&&(P=a),"button-mouseover"==b&&(B=a),"button-mouseout"==b&&(S=a),"button-dropdown-show"==b&&(H=a),"button-dropdown-hide"==b&&(q=a),"button-dropdown-click"==b&&(V=a)
},settings:function(a){null!=a.license&&(Q=a.license),null!=a.css&&(a.css?aa=!0:(aa=!1,addeventatc.removeelement($d("ate_css")))),null!=a.mouse&&(ac=a.mouse),/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)&&(ac=!1),null!=a.outlook&&null!=a.outlook.show&&(ad=a.outlook.show),null!=a.google&&null!=a.google.show&&(ae=a.google.show),null!=a.yahoo&&null!=a.yahoo.show&&(aj=a.yahoo.show),null!=a.hotmail&&null!=a.hotmail.show&&(af=a.hotmail.show),null!=a.outlookcom&&null!=a.outlookcom.show&&(af=a.outlookcom.show),null!=a.ical&&null!=a.ical.show&&(Z=a.ical.show),null!=a.appleical&&null!=a.appleical.show&&(Z=a.appleical.show),null!=a.facebook&&null!=a.facebook.show&&(K=a.facebook.show),null!=a.outlook&&null!=a.outlook.text&&(G=a.outlook.text),null!=a.google&&null!=a.google.text&&(ab=a.google.text),null!=a.yahoo&&null!=a.yahoo.text&&(j=a.yahoo.text),null!=a.hotmail&&null!=a.hotmail.text&&(al=a.hotmail.text),null!=a.outlookcom&&null!=a.outlookcom.text&&(al=a.outlookcom.text),null!=a.ical&&null!=a.ical.text&&(J=a.ical.text),null!=a.appleical&&null!=a.appleical.text&&(J=a.appleical.text),null!=a.facebook&&null!=a.facebook.text&&(F=a.facebook.text),null!=a.dropdown&&null!=a.dropdown.order&&(M=a.dropdown.order)
},keyboard:function(k,s){var r=document.getElementById(s.id+"-drop");
if(r&&"block"==addeventatc.getstyle(r,"display")){for(var f=r.getElementsByTagName("SPAN"),b=null,m=0,p=0,h=0;
h<f.length;
h+=1){m++,addeventatc.hasclass(f[h],"drop_markup")&&(b=f[h],p=m)
}null===b?p=1:"down"==s.key?p>=m?p=1:p++:1==p?p=m:p--,m=0;
for(h=0;
h<f.length;
h+=1){if(++m==p){f[h].className+=" drop_markup"
}else{var g=new RegExp("(\\s|^)drop_markup(\\s|$)");
f[h].className=f[h].className.replace(g," "),f[h].className=f[h].className.replace(/\s+$/,"")
}}}},keyboardclick:function(g,f){var b=document.getElementById(f.id+"-drop");
if(b){for(var m=b.getElementsByTagName("SPAN"),k=null,h=0;
h<m.length;
h+=1){addeventatc.hasclass(m[h],"drop_markup")&&(k=m[h])
}if(null!==k){k.click();
for(h=0;
h<m.length;
h+=1){var l=new RegExp("(\\s|^)drop_markup(\\s|$)");
m[h].className=m[h].className.replace(l," "),m[h].className=m[h].className.replace(/\s+$/,"")
}}}},usewebcal:function(){var f=!1,c=window.navigator.userAgent.toLowerCase(),b=/CriOS/i.test(navigator.userAgent),k=/instagram/.test(c),h=/iphone|ipod|ipad/.test(c),g=/(iPhone|iPod|iPad).*AppleWebKit(?!.*Safari)/i.test(navigator.userAgent);
return(h&&g||h&&b||h&&k)&&(f=!0),f
},agent:function(){var a=navigator.userAgent||navigator.vendor||window.opera;
return/windows phone/i.test(a)?"windows_phone":/android/i.test(a)?"android":/iPad|iPhone|iPod/.test(a)&&!window.MSStream?"ios":"unknown"
}}
}();
!function(g,f){g=g||"docReady",f=f||window;
var b=[],l=!1,j=!1;
function h(){if(!l){l=!0;
for(var a=0;
a<b.length;
a++){b[a].fn.call(window,b[a].ctx)
}b=[]
}}function k(){"complete"===document.readyState&&h()
}f[g]=function(c,a){if("function"!=typeof c){throw new TypeError("callback for docReady(fn) must be a function")
}l?setTimeout(function(){c(a)
},1):(b.push({fn:c,ctx:a}),"complete"===document.readyState||!document.attachEvent&&"interactive"===document.readyState?setTimeout(h,1):j||(document.addEventListener?(document.addEventListener("DOMContentLoaded",h,!1),window.addEventListener("load",h,!1)):(document.attachEvent("onreadystatechange",k),window.attachEvent("onload",h)),j=!0))
}
...
},addeventReady(function(){addeventatc.initialize(),setTimeout("addeventatc.initialize();",200)
});
var SfdcWwwBase=SfdcWwwBase||{};
SfdcWwwBase.bodyCopyComponent=SfdcWwwBase.bodyCopyComponent||{};
(function(a){a(document).ready(function(){SfdcWwwBase.bodyCopyComponent.removeSpaceBetweenHyperlinkAndPunctuation()
});
SfdcWwwBase.bodyCopyComponent.removeSpaceBetweenHyperlinkAndPunctuation=function(){a(".bodyCopyComponent div").each(function(){SfdcWwwBase.bodyCopyComponent.removeSpaceFromHtmlContentBetweenHyperlinkAndPunctuation(a(this))
})
};
SfdcWwwBase.bodyCopyComponent.removeSpaceFromHtmlContentBetweenHyperlinkAndPunctuation=function(b){var c=a(b).html().replace(/(<\/a>)(\s+)([,.;:])/g,"$1$3");
a(b).html(c)
}
}(jQuery));
var SfdcWwwBase=SfdcWwwBase||{};
...
a("body").scrollTo(d,"slow",{offset:-(b.offset-1)})
})
});
...
SfdcWwwBase.utils.getHashParameterByName=function(h){var l=h||"";
l=l.replace(/[\[]/,"\\[").replace(/[\]]/,"\\]");
var k=new RegExp("[\\?&#]"+l+"=([^&#]*)"),j=k.exec(location.hash);
return !j?"":decodeURIComponent(j[1].replace(/\+/g," "))
};
...
SfdcWwwBase.Nav.init=function(){var g=a(".primaryMenuOverlayEffectTypeLevel").length>0;
if(!g){var c=this;
var d=200;
var f=_.throttle(function(){c.equalizeHeight();
if(a(".navbar-expandable-container").height()){c.setDrawerHeight()
}},d);
this.equalizeHeight();
c.bindClick();
a(window).on("resize",f)
}var e=a(".tab-equalize");
var b=0;
a(".overlayMenuItem").hover(function(){var l=document.getElementsByClassName("subOverlayMenu");
var n=new Event("keydown",{bubbles:true,cancelable:true});
n.key="Escape";
l[0].dispatchEvent(n);
b++;
SfdcWwwBase.utils.equalizeHeights(e);
var k=a(this).find(".tabs-header").hasClass("tabs-vertical");
if(a(this).find("div.tabsContainer").length!==0&&k){if(!a(this).find(".globalnavbar-expandable-container").hasClass("vertical-nav")){a(this).find(".globalnavbar-expandable-container").addClass("vertical-nav")
}var j=a(this).find(".tabs-vertical > div:nth-child(1)").width();
var m=a(this).find(".tabs-vertical").hasClass("sideLinks");
var o=a(this).attr("id");
if(m){var h=a(this).find(".tabs-vertical > div:nth-child(2)").width();
a(this).find(".subOverlayMenu").width(j+h+9).css("overflow","hidden")
}else{if(o==="solutions_menu_item"){a(this).find(".subOverlayMenu").width(j+20).css("overflow","hidden")
}else{a(this).find(".subOverlayMenu").width(j+9).css("overflow","hidden")
}}}});
a("[data-toggle=tab]").on("shown.bs.tab",function(n){if(a(this).parent().hasClass("no-content")){var m=a(this).closest(".subOverlayMenu");
var j=a(this).closest(".overlayMenuItem").attr("id");
var k=m.find(".tabs-vertical > div:nth-child(1)").width();
var l=m.find(".tabs-vertical").hasClass("sideLinks");
if(l){var h=m.find(".tabs-vertical > div:nth-child(2)").width();
m.width(k+h+9).css("overflow","hidden")
}else{if(j==="solutions_menu_item"){m.width(k+20).css("overflow","hidden")
}else{m.width(k+9).css("overflow","hidden")
}}}else{a(".subOverlayMenu").removeAttr("style")
}});
this.toggleNav()
};
SfdcWwwBase.Nav.accessibilityInit=function(){try{var d=document.querySelectorAll(".globalnavbar .overlayMenuItem")||[];
Array.prototype.forEach.call(d,function(h){var g=g||0;
var k=h.getElementsByClassName("subOverlayMenu");
var j=document.getElementsByClassName("subOverlayMenu");
var e=k[0].querySelectorAll(".nav-tabs li")[0];
h.addEventListener("keydown",function(n){var m=n.key;
if(m==="Enter"){if(g===0&&e){e.click();
g++
}k[0].style.display="block";
k[0].style.width=null;
k[0].style.overflow=null;
var l=a(".tab-equalize");
SfdcWwwBase.utils.equalizeHeights(l);
k[0].setAttribute("aria-expanded","true");
h.setAttribute("aria-expanded","true")
}if(m==="Escape"){k[0].style.display=null;
h.setAttribute("aria-expanded","false");
k[0].setAttribute("aria-expanded","false")
}});
h.addEventListener("focus",function(l){l.stopPropagation();
Array.prototype.forEach.call(j,function(m){m.style.display=null
})
});
h.addEventListener("focusout",function(){h.setAttribute("aria-expanded","false");
k[0].setAttribute("aria-expanded","false")
})
});
var b=document.querySelectorAll(".nav-tabs > li")||[];
Array.prototype.forEach.call(b,function(e){e.setAttribute("tabindex","0");
e.addEventListener("keydown",function(l){var j=l.key;
if(j==="Enter"){this.click();
var h=document.querySelectorAll(".tab-pane.active")[0];
h.setAttribute("tabindex","-1");
var k=h.querySelectorAll("button, [href], input, select, textarea");
var g=k[0];
g.focus()
}})
});
var c=document.querySelectorAll('[role="navigation"] .tab-content > .tab-pane')||[];
Array.prototype.forEach.call(c,function(g){var h=g.querySelectorAll("button, [href], input, select, textarea");
var e=h[0];
var j=h[h.length-1];
if(e){e.addEventListener("keydown",function(l){if(l.key==="Tab"){if(l.shiftKey){l.stopPropagation();
var k=document.querySelectorAll(".nav-tabs > li.active")[0];
k.nextElementSibling.focus()
}else{return
}}});
j.addEventListener("keydown",function(l){if(l.key==="Tab"){var k=document.querySelectorAll(".nav-tabs > li.active")[0];
k.focus();
l.stopPropagation()
}})
}})
...
SfdcWwwBase.Nav.accessibilityInit()
})
}(jQuery));
var SfdcWwwBase=SfdcWwwBase||{};
...
b(".footerNavigationComponent .region-selector_button").keyup(function(d){if(d.keyCode===13){b(".footerNavigationComponent .region-selector_button").click();
b(".footerNavigationComponent .region-selector_dialog a:any-link").first().focus()
}});
b("body").on("click",c,function(){b(this).prev(a).toggle()
...
}a("body").on("click","a.chat-trigger-v2",function(){var j=[];
if(a(this).hasClass("chat-anchor-tag-contact-us")){j.push("chat-anchor-tag-contact-us")
}if(a(".sidebarHeader.minimizedContainer.isMinimized").length){a(".sidebarHeader.minimizedContainer.isMinimized")[0].click()
}if(a(".helpButtonEnabled").length&&!a(".sidebarHeader.minimizedContainer.isMinimized").length){a(".helpButtonEnabled").trigger("click",j)
}})
};
...
var SfdcWwwBase=SfdcWwwBase||{};
SfdcWwwBase.modal=(function(){var g=1000,r=1.5,f=r*g,m="slide",c="fade",s;
var p=function(){var t;
if(!$("#modal").length){t=$('<div id="modal" class="live-chat-modal"></div>');
$("body").append(t)
}else{t=$("#modal")
}return t
};
var n=function(){return p().is(":visible")
};
var e=function(u){var t=p();
switch(u){case m:t.slideDown(f);
break;
case c:t.fadeIn();
break;
default:t.show();
...
var h=function(u){var t=p();
switch(u){case m:t.slideUp(f);
break;
case c:t.fadeOut();
break;
default:t.hide();
...
var l=function(u,v){var t=u+".html"+location.search;
$.get(t).done(function(w){p().html(w)
}).done(v)
};
var d=function(u,t,v){l(u,v||function(){e(t)
})
};
var j=function(){$("body").on("click","a.open-modal",function(){d($(this).attr("href"))
})
};
var b=function(){$("body").on("click","a.close-modal, a.chat-trigger, a.chat-trigger-v2",function(){h($(this).data("animation"))
})
};
var a=function(){var u=SfdcWwwBase.config.liveChatModalPath;
var w=SfdcWwwBase.config.liveChatWaterFallDelay||30;
var t=w*g;
if(u){var v=typeof digitalData==="object"&&digitalData.user[0]&&digitalData.user[0].profile[0]&&digitalData.user[0].profile[0].usertype;
d(u,m,function(){SfdcWwwBase.liveagent.initSecondButton();
setTimeout(function(){if(!$(".embeddedServiceSidebar").length){e(m);
...
var k=function(){return !!SfdcWwwBase.config&&!SfdcWwwBase.config.liveChatWaterfallDisabled&&typeof digitalData!=="undefined"&&digitalData.localStorage&&!digitalData.localStorage.chat
};
var o=function(){return window.matchMedia("(max-width: 1023px)").matches
};
var q=function(){p();
j();
b();
...
s={showModal:e,hideModal:h,triggerModal:d,isActive:n,shouldDoWaterfall:k,init:q};
if(!SfdcWwwBase.utils.isEditMode()){s.init()
}return s
}());
var SfdcWwwBase=SfdcWwwBase||{};
...
SfdcWwwBase.navigation.handleContactUs=function(){if(!b(".skinnynav .dropdown").nextAll().hasClass("separator")){b(".skinnynav .dropdown").addClass("skinnynav-contact-us")
}b(".globalnavbar .dropdown, .skinnynav .dropdown").hover(function(){b(this).find(".dropdown-menu").show()
...
SfdcWwwBase.navigation.tabAccessibility=function(){var d=b(".globalnavbar .coveo-search-section");
d.on("focus",function(){d.addClass("active")
})
};
SfdcWwwBase.navigation.dropdownAccessibility=function(){var e=document.querySelectorAll(".globalnavbar .dropdown, .skinnynav .dropdown");
var d=document.querySelectorAll(".globalnavbar .region-selector");
b(".globalnavbar .dropdown").on("focus",function(f){b(this).mouseover();
b(this).attr("aria-expanded","true")
});
b(".globalnavbar .dropdown").on("keyup",function(f){if(f.key==="Enter"&&f.target===f.currentTarget){b(this).find("a:any-link")[0].click()
}});
Array.prototype.forEach.call(e,function(f){f.addEventListener("keydown",function(h){var g=h.key;
if(g==="Enter"){b(this).mouseover();
b(this).attr("aria-expanded","true")
}if(g==="Escape"){b(this).mouseout();
b(this).attr("aria-expanded","false")
}});
f.addEventListener("focusout",function(h){var j=h.currentTarget;
var g=h.relatedTarget;
if(!j.contains(g)){b(this).mouseout();
b(this).attr("aria-expanded","false")
}})
});
Array.prototype.forEach.call(d,function(f){f.addEventListener("keydown",function(h){var g=h.key;
if(g==="Enter"){b(this).find(".region-selector_button").mouseover();
b(this).attr("aria-expanded","true")
}if(g==="Escape"){b(this).find(".region-selector_button").mouseout();
b(this).attr("aria-expanded","false");
b(this).focus()
}});
f.addEventListener("focusout",function(h){var j=h.currentTarget;
var g=h.relatedTarget;
if(!j.contains(g)){b(this).find(".region-selector_button").mouseout();
b(this).attr("aria-expanded","false")
}})
})
};
...
SfdcWwwBase.navigation.dropdownAccessibility();
SfdcWwwBase.navigation.tabAccessibility()
};
...
a(".free-trial-mobile-nav .btn").on("click",function(f){var c=a(".free-trial-mobile-nav .hidden-md.hidden-lg .btn"),d=c.text();
f.preventDefault();
...
SfdcWwwBase.mobileNavigation.navZoomAccessibility()
});
...
SfdcWwwBase.mobileNavigation.navZoomAccessibility=function(){var c=a(".mobile-navigation-drawer-active .navigation-container.drawer-container");
var b=a(".mobile-navigation-drawer-active .dynamic-links.generic-links");
if(b.height()>c.height()){c.css("overflow-y","scroll")
}else{c.css("overflow-y","hidden")
}};
...
(function(e){var c="#mainModal",j="form[data-sfdc-form]",h=c+" "+j,d=e(c),g=d.find(".modal-dialog"),b=d.find(".modal-content > .close > span"),f=d.find(".modal-body");
SfdcWwwBase.modalOverlay.mainPayloads={};
SfdcWwwBase.modalOverlay.modalProperties={};
SfdcWwwBase.modalOverlay.setModalProps=function(m){var o=m.data("modal-src")||"",p=m.data("modal-title")||"",k=m.data("modal-backdrop-color")||"",n=m.data("modal-close-button-color")||"",l=m.data("modal-width")||"";
this.modalProperties={path:o,title:p,backdropColor:k,closeButtonColor:n,modalSize:l}
};
SfdcWwwBase.modalOverlay.applyModalStyles=function(){if(Object.keys(this.modalProperties).length){g.addClass(this.modalProperties.modalSize);
b.addClass(this.modalProperties.closeButtonColor);
var k=this;
setTimeout(function(){var l=e(".modal-backdrop");
if(l.length>0){l.addClass(k.modalProperties.backdropColor)
}},50)
}};
SfdcWwwBase.modalOverlay.removeModalStyles=function(){if(Object.keys(this.modalProperties).length){g.removeClass(this.modalProperties.modalSize);
b.removeClass(this.modalProperties.closeButtonColor);
d.find(".modal-backdrop").removeClass(this.modalProperties.backdropColor);
this.modalProperties={}
}};
SfdcWwwBase.modalOverlay.setModalPayload=function(){if(typeof this.mainPayloads[this.modalProperties.path]!=="undefined"){this.loadModalContent(this.mainPayloads[this.modalProperties.path]);
this.applyModalStyles()
}else{var k=this;
e.get(this.modalProperties.path,function(l){k.mainPayloads[k.modalProperties.path]=l;
k.loadModalContent(l)
}).done(k.applyModalStyles())
}};
SfdcWwwBase.modalOverlay.loadModalContent=function(k){this.checkModalVisibility();
f.html(k).find(".columnContainer > .columns-wrapper > .container").removeClass("container").addClass("container-fluid")
};
SfdcWwwBase.modalOverlay.bindUI=function(){var k=this;
e(".image-modal").on("show.bs.modal",function(m){k.setModalProps(e(m.relatedTarget));
if(!a(k.modalProperties.path)){k.modalProperties.path+=".html"
}var l=e(this);
l.find(".modal-title").html(k.modalProperties.title);
l.find(".modal-body-container").load(k.modalProperties.path)
});
d.on("show.bs.modal",function(l){k.setModalProps(e(l.relatedTarget));
k.setModalPayload()
}).on("shown.bs.modal",function(){var l=d.find(j);
if(l.length){SfdcForms.init(h);
l.on("close-parent",function(){if(d.hasClass("in")){d.modal("hide")
}})
}}).on("hidden.bs.modal",function(){if(d.find(j).length){SfdcForms.forms.pop()
}f.html("");
if(typeof window.Dialog.focusedElBeforeOpen!=="undefined"){window.Dialog.focusedElBeforeOpen.focus()
}k.removeModalStyles()
});
e(".modal").on("click","[data-target='#mainModal']",function(l){e(this).removeAttr("data-toggle");
k.setModalProps(e(l.currentTarget));
k.setModalPayload()
})
};
SfdcWwwBase.modalOverlay.modalTabbing=function(){var l=this;
window.Dialog={};
var k=window.Dialog;
k.modalContent=document.querySelector(".modal-content");
k.focusableEls=k.modalContent.querySelectorAll('a[href], area[href], iframe, input:not([disabled]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), [tabindex="0"]');
k.focusedElBeforeOpen=document.activeElement;
k.firstFocusableEl=k.focusableEls[0];
k.lastFocusableEl=k.focusableEls[k.focusableEls.length-1];
k.firstFocusableEl.focus();
k.modalContent.addEventListener("keydown",function(m){l._handleKeyDown(m)
})
};
SfdcWwwBase.modalOverlay.checkModalVisibility=function(){var l=20;
var m=this;
function k(){var o=f.is(":visible");
var n=f.children().length;
if(typeof o!=="undefined"&&o&&n){m.modalTabbing()
}else{if(l>0){l--;
setTimeout(function(){k()
},500)
}}}k()
};
SfdcWwwBase.modalOverlay._handleKeyDown=function(o){var k=window.Dialog;
var p=9;
var n=27;
function m(){if(document.activeElement===k.firstFocusableEl){o.preventDefault();
k.lastFocusableEl.focus()
}}function l(){if(document.activeElement===k.lastFocusableEl){o.preventDefault();
k.firstFocusableEl.focus()
}}switch(o.keyCode){case p:if(k.focusableEls.length===1){o.preventDefault();
break
}if(o.shiftKey){m()
}else{l()
}break;
case n:e(".modal-content button.close")[0].click();
break;
default:break
}};
function a(k){return !k||k.trim()===""||k.indexOf("?")>=0||k.indexOf("#")>=0||k.indexOf(".html")>=0||k.indexOf("http://")===0||k.indexOf("https://")===0||k.lastIndexOf("/")===k.length-"/".length
}e(document).ready(function(){SfdcWwwBase.modalOverlay.bindUI()
})
}(jQuery));
var SfdcWwwBase=SfdcWwwBase||{};
...
}c.get(d,function(k){var f=c.parseHTML(k,true),j=c(f).find(".filters"),g=c(f).find(".event-pages-data"),l=c(f).find(".alert-success"),h=c(f).find(".no-result");
if(!b){c(".filteredCardsAjax-spinner .slds-spinner_container").addClass("hide")
}c(".filters").replaceWith(j);
c(".event-pages-data").replaceWith(g);
c(".alert-success").replaceWith(l);
c(".no-result").replaceWith(h)
...
if(!a(d).parent().hasClass("no-content")){a(".subOverlayMenu").removeAttr("style")
}},b)
...
var SfdcWwwBase=SfdcWwwBase||{};
SfdcWwwBase.pricingComparisonComponentV2=SfdcWwwBase.pricingComparisonComponentV2||{};
(function(a){SfdcWwwBase.pricingComparisonComponentV2.equalizeCells=function(){SfdcWwwBase.utils.equalizeHeights(".pricingComparisonComponentV2 .feature-name-group, .pricingComparisonComponentV2 .feature-amount");
a(".pricingComparisonComponent .panel-group").on("shown.bs.collapse",function(){SfdcWwwBase.utils.equalizeHeights(".pricingComparisonComponentV2 .feature-included, .pricingComparisonComponentV2 .feature-name")
})
};
SfdcWwwBase.pricingComparisonComponentV2.tracking=function(c){if(typeof(digitalData)!=="undefined"&&digitalData.util&&digitalData.util.trackActivity){var b=Page.getName();
var e=typeof digitalData==="object"&&digitalData.user[0]&&digitalData.user[0].profile[0]&&digitalData.user[0].profile[0].visitnumber;
var d={properties:{eVar8:b?"D=pageName":"",eVar34:e,eVar35:Page.getCloud(),prop49:Page.getSegment()}};
a.extend(d.properties,c);
digitalData.util.trackActivity(b,d)
}};
SfdcWwwBase.pricingComparisonComponentV2.toolTips=function(){a('.pricingComparisonComponentV2 [data-toggle="popover"]').popover();
a(".pricingComparisonComponentV2 [data-toggle=popover]").on("shown.bs.popover",function(){a(".pricingComparisonComponentV2 .popover").css("top",parseInt(a(".popover").css("top"))+25+"%")
})
};
SfdcWwwBase.pricingComparisonComponentV2.expandCollapse=function(){a(".pricingComparisonComponentV2 a.expand-all").click(function(){a(this).hide();
a(this).parent().parent().find(".panel-collapse").collapse("show");
a(this).closest(".header-bar").children(".collapse-all").show();
SfdcWwwBase.pricingComparisonComponentV2.tracking({eVar37:"pricing_feature_comparison_table:collapse_all"})
});
a(".pricingComparisonComponentV2 a.collapse-all").click(function(){a(this).hide();
a(this).parent().parent().find(".panel-collapse").collapse("hide");
a(this).closest(".header-bar").children(".expand-all").show()
});
a('.pricingComparisonComponentV2 a[data-toggle="collapse"]').click(function(){if(a(this).attr("aria-expanded")==="false"){var b=a(this).find("h4").text();
b=b.replace(/ /g,"_").replace(/\./g,"").toLowerCase();
SfdcWwwBase.pricingComparisonComponentV2.tracking({eVar37:"pricing_feature_comparison_table:"+b})
}})
};
SfdcWwwBase.pricingComparisonComponentV2.stickyRelocate=function(){var b=a(window).scrollTop();
var e=a("[class^=sticky-anchor]").offset().top;
var d=a("[class^=sticky-anchor-end]").offset().top;
a("[class^=sticky-anchor-end]").each(function(){if(a(this).offset().top>d){d=a(this).offset().top
}});
var c=a(".pricing-comparison-feature-category").width();
if(b>e&&b<d){a(".feature-comparison-header").each(function(f){if(f===0){a(this).addClass("sticky");
a(this).height(a(this).outerHeight())
}else{a(this).addClass("hide")
}a(this).css("width",c)
})
}else{a(".feature-comparison-header").each(function(f){if(f===0){a(this).removeClass("sticky")
}})
}};
SfdcWwwBase.pricingComparisonComponentV2.init=function(){SfdcWwwBase.pricingComparisonComponentV2.equalizeCells();
SfdcWwwBase.pricingComparisonComponentV2.toolTips();
SfdcWwwBase.pricingComparisonComponentV2.expandCollapse();
a(window).scroll(SfdcWwwBase.pricingComparisonComponentV2.stickyRelocate);
a(window).resize(function(){SfdcWwwBase.pricingComparisonComponentV2.equalizeCells()
})
};
a(document).ready(function(){if(a(".pricingComparisonComponentV2").length>0){SfdcWwwBase.pricingComparisonComponentV2.init()
}})
}(jQuery));
var SfdcWwwBase=SfdcWwwBase||{};
...
var d="AgentStatus";
var b;
SfdcWwwBase.livechat.trackSnapInWindowOpensFromChatAnchorOrFall=function(){e("body").on("click","a.uiButton",function(h,g){b=h.isTrigger?"chat-fall":"chat-anchor";
if(g==="chat-anchor-tag-contact-us"){b="contact-us"
...
}else{if(e(".embeddedServiceHelpButton").css("display")==="none"&&!e(".embeddedServiceSidebar").length){e(".embeddedServiceHelpButton").show();
...
}setInterval(function(){if(a("liveagent_vc")!==""){if(e(".helpButton").length&&e(".helpButton .message").text()!=="Agent Offline"){e("#liveagent_button_offline_"+j).hide();
e("#liveagent_button_v2_"+j).show();
e("#liveagent_button_online_"+j+"1").show();
e("#liveagent_button_offline_"+j+"1").hide();
if(e(".embeddedServiceSidebar").length>0){e(".embeddedServiceHelpButton").hide()
}else{e(".embeddedServiceHelpButton").show()
}}}},1000)
});
...
if(/Android|webOS|iPhone|iPad|iPod/i.test(navigator.userAgent)){e("body").addClass("la-mobile")
}})
}(jQuery));
var SfdcWwwBase=SfdcWwwBase||{};
...
(function(f){SfdcWwwBase.jumbotronComponent.swapElements={};
SfdcWwwBase.jumbotronComponent.swapPath="";
SfdcWwwBase.jumbotronComponent.mainPayloads={};
var b=".swap-container",d=".swap-content-wrapper",c=".swap-container-content",e="form[data-sfdc-form]";
var a=function(){return window.matchMedia("(max-width: 767px)").matches
};
SfdcWwwBase.jumbotronComponent.targetInit=function(){f(window).trigger("resize")
};
SfdcWwwBase.jumbotronComponent.initSwapContainer=function(){if(SfdcWwwBase.utils.isEditMode()||a()){return
}this.setSwapElements();
if(!this.swapElements.swapContainer){return
}var g=this.swapElements.swapContainer.querySelectorAll("a[data-target]");
var k=Array.prototype.slice.call(g);
var j=this;
k.forEach(function(l){l.removeAttribute("data-target");
l.onclick=function(){j.swapPath=l.getAttribute("data-modal-src")||"";
j.setSwapPayload()
}
});
var h=this.swapElements.swapWrapper.querySelector(".close");
h.onclick=function(){j.toggleSwapContainer()
};
f(document).keyup(function(l){if(l.keyCode===27){j.toggleSwapContainer()
}})
};
SfdcWwwBase.jumbotronComponent.setSwapElements=function(){var j=document.querySelector(b),h=j?j.querySelector(d):null,g=h?h.querySelector(c):null;
this.swapElements={swapContainer:j,swapWrapper:h,swapContent:g}
};
SfdcWwwBase.jumbotronComponent.loadSwapContent=function(h){this.swapElements.swapContent.innerHTML=h;
var g=this.swapElements.swapContent.querySelector(".columnContainer > .columns-wrapper > .container");
if(g){g.classList.add("container-fluid");
g.classList.remove("container")
}this.toggleSwapContainer(true);
if(this.swapElements.swapContainer.querySelector(e).length){SfdcForms.init(b+" "+e)
}};
SfdcWwwBase.jumbotronComponent.setSwapPayload=function(){if(typeof this.mainPayloads[this.swapPath]!=="undefined"){this.loadSwapContent(this.mainPayloads[this.swapPath])
}else{var g=this;
f.get(g.swapPath,function(h){g.mainPayloads[SfdcWwwBase.jumbotronComponent.swapPath]=h;
g.loadSwapContent(h)
})
}};
SfdcWwwBase.jumbotronComponent.toggleSwapContainer=function(g){var k="swap-hidden";
var l="swap-visible";
if(g){this.swapElements.swapWrapper.classList.remove(k);
var j=this.swapElements.swapWrapper.clientHeight;
this.swapElements.swapContainer.previousHeight=this.swapElements.swapContainer.clientHeight;
this.swapElements.swapContainer.style.minHeight=j+"px";
this.swapElements.swapContainer.style.maxHeight=j+"px";
this.swapElements.swapWrapper.classList.add(l)
}else{if(this.swapElements.swapWrapper.classList.contains(l)){this.swapElements.swapWrapper.classList.remove(l);
this.swapElements.swapContainer.style.minHeight=this.swapElements.swapContainer.previousHeight+"px";
this.swapElements.swapContainer.style.maxHeight="";
var h=this;
setTimeout(function(){h.swapElements.swapWrapper.classList.add(k)
},500)
}}};
f(document).ready(function(){SfdcWwwBase.jumbotronComponent.initSwapContainer()
})
}(jQuery));
var SfdcWwwBase=SfdcWwwBase||{};
SfdcWwwBase.redirectConfirm=SfdcWwwBase.redirectConfirm||{};
(function(a){SfdcWwwBase.redirectConfirm.sanitizeRedirectPath=function(c){var b=document.createElement("div");
b.appendChild(document.createTextNode(c));
return b.innerHTML
};
SfdcWwwBase.redirectConfirm.getParam=function(e){var b="[\\?&]+"+e+"=([^&#]*)";
var d=new RegExp(b,"i");
var c=d.exec(window.location.href);
if(c===null){return""
}else{return decodeURIComponent(c[1])
}};
SfdcWwwBase.redirectConfirm.getRedirectUrl=function(){var c="https://www.salesforce.com",b=SfdcWwwBase.redirectConfirm.getParam("url");
if(b){b=SfdcWwwBase.redirectConfirm.sanitizeRedirectPath(b)
}return b||c
};
SfdcWwwBase.redirectConfirm.init=function(){var d=SfdcWwwBase.redirectConfirm.getRedirectUrl(),c="#safe-redirect-link",b=a(c);
if(b){b.attr("href",d.replace("&amp;","&"));
b.find("span").html(d)
}};
SfdcWwwBase.redirectConfirm.init()
})(jQuery);
var SfdcWwwBase=SfdcWwwBase||{};
...
}else{if(b==="newestSort"||b==="onTime"){return"dateForSorting"
}else{if(b==="oldestSort"){return"dateForSortingOld"
}else{if(b==="popularSort"){return"popularity"
}else{if(b==="createdDate"||b==="modifiedDate"){return b
}else{return SfdcWwwBase.customListComponent.sortKey
}}}}}};
...
}if(SfdcWwwBase.customListComponent.sortKey==="dateForSorting"||SfdcWwwBase.customListComponent.sortKey==="createdDate"||SfdcWwwBase.customListComponent.sortKey==="modifiedDate"){SfdcWwwBase.customListComponent.totalResultSet.sort(function(d,c){if(Date.parse(d[SfdcWwwBase.customListComponent.sortKey])===null&&Date.parse(c[SfdcWwwBase.customListComponent.sortKey])===null){return 0
...
if(!SfdcWwwBase.customListComponent.isShowMoreEnabled()){SfdcWwwBase.customListComponent.gridContainerHtml+=j[0].outerHTML
}else{g.append(j)
}}else{if(!SfdcWwwBase.customListComponent.isShowMoreEnabled()){SfdcWwwBase.customListComponent.gridContainerHtml+=h
}else{g.append(h)
...
}if(SfdcWwwBase.customListComponent.currentNumberOfResults===SfdcWwwBase.customListComponent.pageSize||d===SfdcWwwBase.customListComponent.resultSet.length-1){if(!SfdcWwwBase.customListComponent.isShowMoreEnabled()){SfdcWwwBase.customListComponent.resultsDiv.find(".filters-grid-container").empty();
SfdcWwwBase.customListComponent.resultsDiv.find(".filters-grid-container").append(SfdcWwwBase.customListComponent.gridContainerHtml);
SfdcWwwBase.customListComponent.gridContainerHtml=""
}a(".pagination-spinner .slds-spinner_container").addClass("hide");
a(".filteredCardsAjax-spinner .slds-spinner_container").addClass("hide")
...
a(".pagination-spinner .slds-spinner_container").removeClass("hide");
...
SfdcWwwBase.customListComponent.bindPrevNextButtons=function(){a(".customListComponent .pagination-first-last a").click(function(d){d.preventDefault();
var c=a(this).data("page");
var b=c;
if(c==="first"||c==="previous"){if(SfdcWwwBase.customListComponent.currentPage===1){return false
}b=(c==="first"?1:SfdcWwwBase.customListComponent.currentPage-1)
}else{if(c==="last"||c==="next"){if(SfdcWwwBase.customListComponent.currentPage===SfdcWwwBase.customListComponent.totalPages){return false
}b=(c==="last"?SfdcWwwBase.customListComponent.totalPages:SfdcWwwBase.customListComponent.currentPage+1)
...
if(b!==""){SfdcWwwBase.customListComponent.sortKey=SfdcWwwBase.customListComponent.applySortKeyForJson(b)
}else{SfdcWwwBase.customListComponent.sortKey=SfdcWwwBase.customListComponent.applySortKeyForJson(SfdcWwwBase.customListComponent.sortKey)
}};
...
SfdcWwwBase.customListComponent.isShowMoreEnabled=function(){return !a(".customListComponent .load-more-container").hasClass("hidden")
};
...
var SfdcWwwBase=SfdcWwwBase||{};
SfdcWwwBase.tabTile=SfdcWwwBase.tabTile||{};
(function(a){SfdcWwwBase.tabTile.resizeTimer=0;
SfdcWwwBase.tabTile.setHeight=function(){var b=0;
if(SfdcWwwBase.tabTile.isDesktop()===true){if(SfdcWwwBase.tabTile.tiles&&SfdcWwwBase.tabTile.tiles.size()>0){SfdcWwwBase.tabTile.tiles.each(function(){if(a(this).width()>b){b=a(this).width()
}});
SfdcWwwBase.tabTile.tiles.each(function(){var c=b;
if(a(this).width()!==b&&SfdcWwwBase.utils.isEditMode()){c+=80
}a(this).css("height",c+"px")
})
}}else{SfdcWwwBase.tabTile.tiles.each(function(){a(this).css("height","")
});
SfdcWwwBase.tabTile.tiles.each(function(){if(a(this).find(".tile-container").height()>b){b=a(this).height()
}});
SfdcWwwBase.tabTile.tiles.each(function(){var c=b;
if(a(this).find(".tile-container").height()!==b&&SfdcWwwBase.utils.isEditMode()){c+=80
}a(this).find(".tile-container").css("height",c+"px")
})
}SfdcWwwBase.tabTile.checkNoImage();
SfdcWwwBase.tabTile.addEllipsis()
};
SfdcWwwBase.tabTile.checkNoImage=function(){var b=a(".tabTileNavigationContainerComponent .nav-item");
b.each(function(){if(!a(this).find("img.img-responsive, .salesforce-icon")){a(this).addClass("no-icon")
}})
};
SfdcWwwBase.tabTile.isDesktop=function(){if(SfdcWwwBase.utils.getCurrentBreakpoint()==="lg"){return true
}else{return false
}};
SfdcWwwBase.tabTile.addEllipsis=function(){var c=a(".desc-container");
var b=a(".title-container");
a.each(b,function(g,e){var d=a(this).find("a");
var f=a(e).outerHeight();
while(d.outerHeight()>f){d.text(d.text().replace(/\W*\s(\S)*$/,"..."))
}});
a.each(c,function(e,d){var g=a(this).find("p");
var f=a(d).outerHeight();
while(g.outerHeight()>f){g.text(g.text().replace(/\W*\s(\S)*$/,"..."))
}})
};
SfdcWwwBase.tabTile.bindTabClick=function(){var b=a(".tile-container.thumbnail-linked");
b.each(function(){a(this).closest(".tabTileComponent").addClass("full-lnked-tab")
});
a("body").on("click",".full-lnked-tab",function(d){var c=a(this).find(".tile-container.thumbnail-linked").attr("data-destination");
window.location=c
})
};
SfdcWwwBase.tabTile.init=function(){SfdcWwwBase.tabTile.tiles=a(".tile-layout .tabTileComponent");
if(SfdcWwwBase.tabTile.tiles.length>0&&!SfdcWwwBase.utils.isEditMode()){SfdcWwwBase.tabTile.setHeight();
SfdcWwwBase.tabTile.bindTabClick();
var b=_.debounce(function(c){SfdcWwwBase.tabTile.setHeight()
},200);
window.addEventListener("resize",b,false)
}};
a(document).ready(function(){SfdcWwwBase.tabTile.init()
})
}(jQuery));
var SfdcWwwBase=SfdcWwwBase||{};
(function(e){if(SfdcWwwBase.utils.isEditMode()){return
}var c=".popup-panel-wrap",a=e(c);
if(!a.length){return
}var b="panel-open",d=["valid","error","active"];
SfdcWwwBase.popupPanel={init:function(){a.find("[data-foot-panel-action]").click(function(h){h.preventDefault();
var j=e(this).parents(c),g=this.dataset.footPanelAction;
switch(g){case"open":j.addClass(b);
break;
case"minimize":j.removeClass(b);
break;
case"close":var f=e(this).find("form");
j.removeClass(b);
if(f.length){f.trigger("reset");
d.forEach(function(k){f.find("."+k).removeClass(k)
})
}break
}});
a.find("form").on("close-parent",function(){e(this).parents(c).removeClass(b)
})
}};
SfdcWwwBase.popupPanel.init()
})(jQuery);
var SfdcWwwBase=SfdcWwwBase||{};
SfdcWwwBase.carouselAccessibilityUtil=SfdcWwwBase.carouselAccessibilityUtil||{};
(function(j){var n;
var p;
var g;
var c;
...
var e=false;
var q=0;
var k="a, button, input, select, option, textarea, [tabindex=0]";
var d;
var a;
var o=9;
var h=13;
SfdcWwwBase.carouselAccessibilityUtil.init=function(){SfdcWwwBase.carouselAccessibilityUtil.disableTabOnSliderInnerEls();
SfdcWwwBase.carouselAccessibilityUtil.setupEvents();
j('<a class="flag" href=""></a>').insertBefore(".carouselContainer");
j('<a class="flag" href=""></a>').insertAfter(".carouselContainer")
};
SfdcWwwBase.carouselAccessibilityUtil.checkIfMactchCarouselEl=function(r){n=j(r.target).closest(".carousel");
if(n.length>0){SfdcWwwBase.carouselAccessibilityUtil.setupTabIndex(r)
}else{SfdcWwwBase.carouselAccessibilityUtil.cleanTabIndex()
}};
SfdcWwwBase.carouselAccessibilityUtil.checkLastElCarousel=function(r){f=r.target;
n=j(r.target).closest(".carousel");
if(n.length>0&&j(r.target).attr("tabindex")==="1"&&a&&r.keyCode===o&&r.shiftKey){r.preventDefault();
a.focus()
}else{if(n.length>0&&j(f).attr("tabindex")===b.toString()&&d&&r.keyCode===o&&!r.shiftKey){SfdcWwwBase.carouselAccessibilityUtil.disableTabOnSliderInnerEls();
r.preventDefault();
d.focus()
}}};
SfdcWwwBase.carouselAccessibilityUtil.jumpToSliderIni=function(s){var r=j(s.target).closest(".carousel");
if(!p){setTimeout(function(){r.find("[tabindex=1]").focus()
},100)
}};
SfdcWwwBase.carouselAccessibilityUtil.cleanTabIndex=function(){if(p){p.removeAttr("tabindex");
SfdcWwwBase.carouselAccessibilityUtil.disableTabOnSliderInnerEls();
g.removeAttr("tabindex");
c.removeAttr("tabindex");
b=0;
j.each(m,function(r,s){j(s).removeAttr("tabindex")
});
p=null;
g=null;
c=null;
...
SfdcWwwBase.carouselAccessibilityUtil.setupEvents=function(){j(document).on("keyup",SfdcWwwBase.carouselAccessibilityUtil.checkIfMactchCarouselEl);
j(document).on("keydown",SfdcWwwBase.carouselAccessibilityUtil.checkLastElCarousel);
j(document).on("click",SfdcWwwBase.carouselAccessibilityUtil.onCLickDocument);
j(".carousel").on("click",".carousel_control-nav a",SfdcWwwBase.carouselAccessibilityUtil.onClickNavItem);
j(".carousel").on("click",".carousel_next, .carousel_prev",SfdcWwwBase.carouselAccessibilityUtil.clearActiveSliderTabIndex);
j(".carousel").on("focus",".carousel_control-nav li:first-child a, .carousel_next",SfdcWwwBase.carouselAccessibilityUtil.jumpToSliderIni);
j(".carousel").on("focus",".carousel_active-slide",SfdcWwwBase.carouselAccessibilityUtil.onFocusActiveSlide);
j(".carousel").on("focus",".carousel_prev",SfdcWwwBase.carouselAccessibilityUtil.jumpToSliderIni)
};
SfdcWwwBase.carouselAccessibilityUtil.onFocusActiveSlide=function(t){var r=j(t.target);
var s=r.find("a, button, input, select, option, textarea, [tabindex=0]");
if(s.length>0){s[0].focus()
}};
SfdcWwwBase.carouselAccessibilityUtil.clearActiveSliderTabIndex=function(r){SfdcWwwBase.carouselAccessibilityUtil.disableTabOnSliderInnerEls();
p.removeAttr("tabindex");
p=j(r.target).closest(".carousel").find(".carousel_active-slide");
p.attr("tabindex",1)
};
SfdcWwwBase.carouselAccessibilityUtil.onClickNavItem=function(r){if(p){p.removeAttr("tabindex");
SfdcWwwBase.carouselAccessibilityUtil.disableTabOnSliderInnerEls();
p=j(r.target).closest(".carousel").find(".carousel_active-slide");
p.attr("tabindex",1);
setTimeout(function(){p.focus()
},300)
}};
SfdcWwwBase.carouselAccessibilityUtil.setupTabIndex=function(t){b=0;
p=n.find(".carousel_active-slide");
g=n.find(".carousel_prev");
c=n.find(".carousel_next");
m=n.find(".carousel_control-nav a");
l=n.find(".carousel_control-nav li:first-child a");
var s=p.find("a, button, input, select, option, textarea, [tabindex=0]");
var r=0;
if(s.length===0){p.attr("tabindex",1);
g.attr("tabindex",2);
c.attr("tabindex",3);
b+=3;
j.each(m,function(u,v){j(v).attr("tabindex",u+4);
b++
})
}else{j.each(s,function(u,v){j(v).attr("tabindex",u+1);
b=u+1
});
g.attr("tabindex",b+1);
c.attr("tabindex",b+2);
b+=2;
j.each(m,function(u,v){j(v).attr("tabindex",u+b+1);
r++
});
b+=r
}SfdcWwwBase.carouselAccessibilityUtil.getNextPrevElAfterCarousel(t)
};
SfdcWwwBase.carouselAccessibilityUtil.getNextPrevElAfterCarousel=function(r){SfdcWwwBase.carouselAccessibilityUtil.findNextClosestTabbableEls(p.closest(".carouselContainer"));
SfdcWwwBase.carouselAccessibilityUtil.finPrevClosestTabbableEls(p.closest(".carouselContainer"))
};
SfdcWwwBase.carouselAccessibilityUtil.finPrevClosestTabbableEls=function(r){a=r.prev()
};
SfdcWwwBase.carouselAccessibilityUtil.findNextClosestTabbableEls=function(r){d=r.next()
};
SfdcWwwBase.carouselAccessibilityUtil.disableTabOnSliderInnerEls=function(){var r=j(".carousel").find(".carousel_slide");
r.find("a, button, input, select, option, textarea, [tabindex=0]").attr("tabindex",-1)
};
SfdcWwwBase.carouselAccessibilityUtil.onCLickDocument=function(r){if(!r.toElement.classList.contains("carousel_prev")&&!r.toElement.classList.contains("carousel_next")&&!r.toElement.classList.contains("carousel_active-slide")&&!r.toElement.classList.contains("carousel_active")){SfdcWwwBase.carouselAccessibilityUtil.cleanTabIndex()
}};
if(j(".carouselContainer").length>0){SfdcWwwBase.carouselAccessibilityUtil.init()
}}(jQuery));
var SfdcWwwBase=SfdcWwwBase||{};
(function(e){if(SfdcWwwBase.utils.isEditMode()){return
}var g="drop-panel",l="data-drop-panel-target-id",m="data-content-src",p=".drop-panel-body",k="data-loaded",j="notransition",d="form[data-sfdc-form]",b="data-disabled",o=function(q){return q.offsetHeight>0
},h=function(q){q.classList.add(j);
q.style.height=q.scrollHeight+"px";
requestAnimationFrame(function(){q.classList.remove(j);
q.style.height="0px"
})
},c=function(q){q.style.height=q.querySelector(p).scrollHeight+"px"
},n=function(q,s){var r=q.getAttribute(m);
e.get(r,function(t){q.querySelector(p).innerHTML=t;
q.setAttribute(k,"true");
if(q.querySelector(d).length){SfdcForms.init(s+" "+d)
}q.addEventListener("transitionend",function(){if(o(q)){q.style.height="auto"
}q.removeAttribute(b)
});
q.querySelector(".close").addEventListener("click",function(){h(q)
});
c(q)
})
},a=function(q,r){if(q.getAttribute(k)==="true"){c(q)
}else{n(q,r)
}},f=function(t){t.preventDefault();
var r=t.currentTarget.getAttribute(l),q="[data-panel-name='"+r+"']",s=document.querySelector(q);
if(s!=="undefined"&&!s.hasAttribute(b)){s.setAttribute(b,"true");
if(o(s)){h(s)
}else{a(s,q)
}}};
SfdcWwwBase.dropPanel={init:function(){var q=document.getElementsByClassName(g);
if(!q.length){return
}Array.prototype.forEach.call(q,function(r){r.addEventListener("click",f)
})
}};
SfdcWwwBase.dropPanel.init()
})(jQuery);