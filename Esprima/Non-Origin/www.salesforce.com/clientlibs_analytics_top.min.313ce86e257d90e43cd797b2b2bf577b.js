...
SfdcWwwBase.gdpr.getGroup=function(i){var j=false;
if(window.Optanon){var k=window.Optanon.GetDomainData().Groups;
k.forEach(function(l){if(l.OptanonGroupId!==undefined&&l.OptanonGroupId===i){j=l
}})
}return j
};
...
SfdcWwwBase.gdpr.getActiveCookies=function(){return h
};
...
g(function(){SfdcWwwBase.gdpr.postInit()
});
...
function CookieHandler(){this.isValid=function(a){if(SfdcWwwBase.gdpr&&SfdcWwwBase.gdpr.isEnabled()){return SfdcWwwBase.gdpr.isCookieActive(a)
}return true
};
this.setCookie=function(c,h,i,g){if(this.isValid(c)){g=g?g:Server.getCookieDomain();
var a="";
var d="";
var b=new Date();
...
this.getCookie=function(b){if(this.isValid(b)){b=b+"=";
...
this.deleteCookie=function(a){if(this.isValid(a)){this.setCookie(a,"",-1);
...
}function Q(S,T){T=T||{bubbles:false,cancelable:false,detail:undefined};
var R=document.createEvent("CustomEvent");
R.initCustomEvent(S,T.bubbles,T.cancelable,T.detail);
return R
}Q.prototype=window.Event.prototype;
window.CustomEvent=Q
})();
var m=false;
...
var o={};
var r=(function(){var R="MockTagManager";
var U="event";
var X="pageLoad";
var T="trackLink";
function W(){return(typeof window[R]!=="undefined")
}function Q(Z){return W()&&typeof window[R][Z]==="function"
}function S(){if(Q(X)){return window[R][X].apply(this,arguments)
}return false
}function V(){if(Q(U)){return window[R][U].apply(this,arguments)
}return false
}function Y(){if(Q(T)){return window[R][T].apply(this,arguments)
}return false
}return{ready:W,load:S,event:V,link:Y}
}());
function a(Q){return !_.isUndefined(Q)&&!_.isNull(Q)
}function t(Q){return !a(Q)||_.isEmpty(Q)
}function x(Q,S){if(!t(Q)){var R=new CustomEvent(Q,{detail:S});
document.dispatchEvent(R);
h(Q)
}}function G(Q,S){if(t(o[Q])){o[Q]={fired:false,stack:[]}
}var R=function(){try{S.apply(arguments)
}catch(T){console.log("Error executing handler for event"+Q)
}};
if(!o[Q].fired){o[Q].stack.push(R)
}else{R()
}}function h(S){if(!t(o[S])){var R=o[S].stack;
function T(){o[S].stack=[];
o[S].fired=true
}function U(V,W){return _.compose(V,W)
}var Q=_.reduce(R,U,T);
Q()
}}function j(Q,R,S){if(r.ready()&&a(Q)&&!t(R)&&a(S)){Q[R]=S
}}function F(U,T){if(!t(U)){var V=/(\w[\w\d]+)\[([\d]+)\]/i;
var S=/(\w[\w\d]+)\[['"]?([\w]+)['"]?\]/i;
function Q(Z,W,aa){if(t(W)){return aa
}else{var ac=W.shift();
if(V.test(ac)){var X=ac.match(V);
var ab=X[1];
var Y=X[2];
if(typeof Z[ab]!=="object"){Z[ab]=[]
}if(!Z[ab][Y]){Z[ab][Y]={}
}if(!W.length){Z[ab][Y]=Q(Z,W,aa)
}else{Q(Z[ab][Y],W,aa)
}}else{if(S.test(ac)){var X=ac.match(S);
var ab=X[1];
var Y=X[2];
if(typeof Z[ab]!=="object"){Z[ab]={}
}if(!Z[ab][Y]){Z[ab][Y]={}
...
}if(!W.length){Z[ac]=Q(Z,W,aa)
}else{Q(Z[ac],W,aa)
}}}}return aa
}var R=U.split(".");
return Q(u,R,T)
}return false
}function C(R,Q){if(r.ready()&&_.isArray(R)&&a(Q)){R.push(Q)
}}function A(Q,S){if(!t(Q)&&!t(S)){j(u.page,Q,S);
var R={path:"page."+Q,value:S};
x(z.ELEMENT_ADDED,R)
}}function I(R){var Q=R;
if(R&&R.toLowerCase()==="legacy"){Q=0
}return u.user[0].profile[Q]
}function w(Q,R){if(!t(Q)&&!t(R)){var S=R;
S.profileID=Q;
if(Q&&Q.toLowerCase()==="legacy"){Q=0
}j(u.user[0].profile,Q,S);
x(z.PROFILE_ADDED,S)
}}function k(R,S){if(!t(R)){var Q={productInfo:{productName:R},category:{primaryCategory:S||""}};
C(u.products,Q);
x(z.PRODUCT_ADDED,Q)
}}function v(R,Q){this.eventInfo={eventName:R,eventID:R,timeStamp:new Date(),data:Q};
if(!t(Q)){this.eventInfo.eventPoints=Q.value||1;
if(!_.isEmpty(Q.primaryCategory)){this.eventInfo.category={primaryCategory:Q.primaryCategory}
}}return this
}function l(Q){return(_.isObject(Q)&&_.has(Q,"eventInfo"))
}function O(R,Q){var S=null;
if(l(R)){S=R
}else{if(_.isString(R)){S=new v(R,Q)
}}return S
}function g(R,Q){var S=O(R,Q);
if(a(S)){r.event(S);
C(u.event,S);
x(z.BEHAVIOR_ADDED,S)
}}function E(S,Q,R){if(!t(S)){r.link(S,_(Q).chain().map(function(T){var U=O(T,{primaryCategory:S,elements:R});
u.util.addBehavior(U);
return U
}).value(),R)
}}function H(Q,S){function R(U,V){if(U&&V.length){var W=V.shift();
return R(U[W],V)
}return U
}if(S&&S.length){var T=S.replace(/\[/g,".").replace(/\]|\'|\"/g,"").split(".");
return R(Q,T)
}return null
}function P(Q){return H(u,Q)
}function q(Q){return H(u.localStorage,Q)||""
}function J(R,T,Q,S){if(R&&T&&R.length>=1&&T.length>=1){Util.setCookie(T,encodeURIComponent(R),Q,S||location.hostname)
}return T
}function D(Q){if(Q&&Q.length>=1){Q=encodeURIComponent(Q);
return Util.getCookie(Q)
}return null
}function L(Q){if(Q&&Q.length>=1){Q=encodeURIComponent(Q);
Util.deleteCookie(Q)
}}function n(T,R,S){var U=D(R);
if(T){var Q=S?(S*86400):void 0;
J(R,T,Q,location.hostname)
}return T==U?"":T
}function d(T,R,S){var Q=S?(S*86400):void 0;
if(T){J(R,T,Q)
}return D(R)
}function i(){if(r.ready()&&!m){r.load();
m=true
}}function B(){h(z.BEFORE_PAGE_LOAD)
}G(z.BEFORE_PAGE_LOAD,i);
function b(R,Q,T){var S=_.reduce(T,function(U,V){var W=u.util.getElement(V);
U.push({element:V,value:W});
return U
},[]);
M();
...
Q=_(Q).chain().map(function(U){return O(U,{primaryCategory:R,elements:T})
}).value(),r.load(Q,T,S)
}function M(){u.page={};
u.user[0].profile={0:{}};
u.event=[];
u.product=[];
u.flags={}
}function c(){if(SfdcWwwBase.gdpr.isEnabled()){if(SfdcWwwBase.gdpr.getActiveCookies().length>0){$.each(u.localStorage,function(Q){if(!SfdcWwwBase.gdpr.isCookieActive(Q)){delete u.localStorage[Q]
}});
...
}function N(T,S,R,X){var V=Page.getLocale();
var Y="";
var Q="";
var W=[];
var Z=[];
...
if(!T){return false
}for(var U in T){var aa=(V+":"+U);
if((Y+aa).length<100){Y+=(Y==""?"":"|")+aa
}if((Q+aa).length<255){Q+=(Q==""?"":"|")+aa
}}if(Y){Y=Y.toLowerCase();
Q=Q.toLowerCase();
u.util.addPageData("formerrors",Y);
Z.push("page.formerrors");
u.util.addPageData("formerrorsall",Q);
Z.push("page.formerrorsall");
u.util.addPageData("formerrorspercentage",Math.round(S/(R+S)*100).toString()+"%");
Z.push("page.formerrorspercentage");
u.util.addBehavior("form_errors");
W.push("form_errors")
}else{u.util.setFormFillVals();
Z.push("user[0].profile[0].usercompanysize");
Z.push("user[0].profile[0].usercompanycountry");
Z.push("user[0].profile[0].userprimaryprodinterest");
Z.push("user[0].profile[0].usercompanyname");
Z.push("user[0].profile[0].userfirstformcompletetime")
}if(X){Page.ajaxFormComplete=true;
u.util.addBehavior("formconfirmationview");
W.push("formconfirmationview");
if(vp.getFormCompleteId()){u.user[0].profile[0].transactionid=vp.getFormCompleteId();
Z.push("user[0].profile[0].transactionid");
u.util.addPageData("numberofformcompletes","+1");
Z.push("page.numberofformcompletes");
u.util.addBehavior("formcompletesubmitbuttonclick");
W.push("formcompletesubmitbuttonclick");
if(Page.isDemo()){u.util.addBehavior("demoformcomplete");
W.push("demoformcomplete")
}if(Page.isSignup()){u.util.addBehavior("signupformcomplete");
W.push("signupformcomplete")
}}}u.util.addBehavior("formbutton_clicks");
W.push("formbutton_clicks");
if(Page.hasChatForm()){u.util.addBehavior("chatview");
W.push("chatview")
}if(W.length===0){W.push("")
}u.util.trackBehavior("Form error tracking",W,Z)
}function p(){u.util.trackBehavior("Ajax form view",[""],[])
}function y(){var Q=["formconfirmationview"];
u.user[0].profile[0].transactionid=vp.getFormCompleteId();
if(u.user[0].profile[0].transactionid){if(u.util.readCookie("c30")){u.page.offerid=u.util.readCookie("c30")
}else{u.page.offerid="[NO OFFER ID]"
}u.page.numberofformcompletes="+1";
Q.push("formcompletesubmitbuttonclick");
Q.push("signupformcomplete")
}u.util.deleteCookie("v0");
u.util.deleteCookie("v20");
u.util.deleteCookie("v22");
u.util.deleteCookie("v28");
u.util.deleteCookie("c30");
u.util.trackBehavior("syncSignupFormCompleted",Q,["user[0].profile[0].usercompanysize","user[0].profile[0].usercompanycountry","user[0].profile[0].userprimaryprodinterest","user[0].profile[0].usercompanyname","user[0].profile[0].userfirstformcompletetime","page.numberofformcompletes","user[0].profile[0].transactionid","page.offerid"])
}function K(Q,R){var S=[];
var T=[];
if(!Q){return false
}if(!R){R="Social sharing"
}if(R==="Social signup"){u.util.addPageData("sharechannel",Q);
T.push("page.sharechannel");
u.util.addPageData("customlink",R);
T.push("page.customlink")
}else{if(R==="Social success"){u.util.addBehavior("social_signin_success");
S.push("social_signin_success")
}else{u.util.addBehavior("socialshares");
S.push("socialshares");
u.util.addPageData("sharechannel",Q);
T.push("page.sharechannel");
R=Q+": social share"
}}if(S.length===0){S.push("")
}u.util.trackBehavior(R,S,T)
}u.util={on:G,pageLoad:B,addPageData:A,getUserProfileByName:I,setUserProfileByName:w,addProduct:k,addBehavior:g,Behavior:v,trackBehavior:E,getElement:P,setElement:F,writeCookie:J,readCookie:D,deleteCookie:L,getValueOnce:n,getAndPersistValue:d,dynamicPageLoad:b,event:z,saveLocalStorage:c,getLocalStorage:q,trackForm:N,trackAjaxForm:p,trackSyncSignupConf:y,trackSocial:K}
}(digitalData));
"use strict";
...
var d=[];
function i(n){return _(n).chain().uniq(false,function(o){return o.productInfo.productName+";"+o.category.primaryCategory
...
if(Page.getDriver()&&Page.getDriver()["id"]==="Typed/Bookmarked"&&Page.getDriver()["type"]==="Typed/Bookmarked"){vp.activityData.d="7010M000002Mw7O"
...
},getBlogArticleTitle:function(){var a=document.querySelector(".articleInfo-title .articleInfo-title-text");
...
},isFormKickback:function(){return Util.getParam("ve")
},isFormPreSyncConfirmPage:function(a){return(a.indexOf(":conf:syncconf")>-1)
...
},isSkipFormRedirect:function(){return(Util.getParam("redirect")==="true"&&Util.getParam("leadcreated")==="true")
...
},isValidLocale:function(a){var b=["ap","au","br","ca","cn","de","dk","es","eu","fi","fr","fr-ca","in","it","jp","kr","mx","ne","nl","no","se","tw","uk","us","my","hk","th","fi","dk","pl","tr"];
return b.indexOf(a)>-1
},isValidServer:function(b){var a=["SFDC","APPX","STOR","LMS","TRUST","DFC","COMM"];
return a.indexOf(b)>-1
},getFormType:function(){var a="";
...
},getFeedbackButton:function(){return $(".opinionlab-trigger, .feedback-trigger")
...
var a={};
digitalData.util.mappings.google={register:function(c,g){if(typeof c==="string"){var d=c.toLowerCase();
if(!a[d]){a[d]=g
}}return this
},getEvent:function(c){if(typeof c==="string"){var d=c.toLowerCase();
var g=a[d];
if(typeof g==="function"){return g()
}}return undefined
}};
digitalData.util.mappings.google.register("chatdrop",function(){return{event:"custEv_chatDrop",eventCat:"Chat",eventAct:"Chat Drop",eventLbl:"Chat Drop",eventVal:0,nonInteraction:1,chatLocation:"Chat Drop",chatViews:"1"}
}).register("chatformshown",function(){var d=digitalData.page.customlink;
var c=d==="chat-fall"?"Waterfall":d==="contact-us"?"Contact menu":"Anchor";
return{event:"custEv_chatView",eventCat:"Chat",eventAct:"Chat View",eventLbl:c,eventVal:0,nonInteraction:0,chatLocation:c,chatInitiations:"1"}
}).register("chatview",function(){var d=digitalData.page.customlink;
var c=d==="chat-fall"?"Waterfall":d==="contact-us"?"Contact menu":"Anchor";
return{event:"custEv_chatInitialize",eventCat:"Chat",eventAct:"Chat Initialize",eventLbl:c,eventVal:0,nonInteraction:1,chatLocation:c,chatViews:"1"}
}).register("bottomnavigationclicks",function(){return{event:"custEv_bottomNavLinkClick",eventCat:"Navigation Link Click",eventAct:"Bottom",eventLbl:b(),eventVal:0,nonInteraction:0}
}).register("topnavigationclicks",function(){if(digitalData.page.moduletracking&&(digitalData.page.customlink===undefined||(digitalData.page.customlink&&digitalData.page.customlink.indexOf("|Top")===-1))){return{event:digitalData.page.moduletracking.indexOf("|left-nav")>-1?"custEv_leftNavLinkClick":"custEv_topNavLinkClick",eventCat:"Navigation Link Click",eventAct:digitalData.page.moduletracking.indexOf("|left-nav")>-1?"Left":"Top",eventLbl:b(),eventVal:0,nonInteraction:0}
}else{return undefined
}}).register("leftnavigationclicks",function(){return{event:"custEv_leftNavLinkClick",eventCat:"Navigation Link Click",eventAct:"Left",eventLbl:b(),eventVal:0,nonInteraction:0}
}).register("formview",function(){return{event:"custEv_formImpression",eventCat:"Form Engagement",eventAct:"Form Views",eventLbl:Page.getFormType(),eventVal:0,nonInteraction:0,formID:digitalData.page.offerid,localizedFormName:Page.getName(),formType:Page.getFormType(),formViews:1}
}).register("formcompletesubmitbuttonclick",function(){return{event:"custEv_formCompletion",eventCat:"Form Engagement",eventAct:"Form Completion",eventLbl:Page.getFormType(),eventVal:0,nonInteraction:0,formID:digitalData.page.offerid,localizedFormName:Page.getName(),formJoinIDUser:digitalData.user[0].profile[0].transactionid,formJoinIDHit:digitalData.user[0].profile[0].transactionid,formType:Page.getFormType(),formCompletions:"1"}
}).register("form_errors",function(){return{event:"custEv_formErrors",eventCat:"Form Engagement",eventAct:"Form Errors",eventLbl:digitalData.page.formerrorsall,eventVal:0,nonInteraction:0,formID:digitalData.page.offerid,localizedFormName:Page.getName(),formType:Page.getFormType(),formErrors:"1"}
}).register("social_signin_success",function(){return{event:"custEv_formFillUsingSocialNetwork",eventCat:"Form Engagement",eventAct:"Form Fill Using Social Account",eventLbl:digitalData.page.sharechannel,eventVal:0,nonInteraction:0,formID:digitalData.page.offerid,formType:Page.getFormType()}
}).register("video_load",function(){return{event:"custEv_videoLoad",eventCat:"Video Engagement",eventAct:digitalData.page.videoname,eventLbl:"Load",eventVal:0,nonInteraction:1,videoLoads:1,videoName:digitalData.page.videoname}
}).register("number_video_views",function(){return{event:"custEv_videoPlay",eventCat:"Video Engagement",eventAct:digitalData.page.videoname,eventLbl:"Play",eventVal:0,nonInteraction:0,videoName:digitalData.page.videoname,videoAction:"play",videoPercentageWatched:"1%",videoSecondsWatched:digitalData.page.videoduration,videoPlays:1}
}).register("video_25percent_viewed",function(){return{event:"custEv_precentComplete",eventCat:"Video Engagement",eventAct:digitalData.page.videoname,eventLbl:"25% Completion",eventVal:0,nonInteraction:0,videoName:digitalData.page.videoname,videoAction:"25% Completion",videoPercentageWatched:"25%",videoSecondsWatched:digitalData.page.videoduration}
}).register("video_50percent_viewed",function(){return{event:"custEv_precentComplete",eventCat:"Video Engagement",eventAct:digitalData.page.videoname,eventLbl:"50% Completion",eventVal:0,nonInteraction:0,videoName:digitalData.page.videoname,videoAction:"50% Completion",videoPercentageWatched:"50%",videoSecondsWatched:digitalData.page.videoduration}
}).register("video_75percent_viewed",function(){return{event:"custEv_precentComplete",eventCat:"Video Engagement",eventAct:digitalData.page.videoname,eventLbl:"75% Completion",eventVal:0,nonInteraction:0,videoName:digitalData.page.videoname,videoAction:"75% Completion",videoPercentageWatched:"75%",videoSecondsWatched:digitalData.page.videoduration}
}).register("video_100percent_viewed",function(){return{event:"custEv_precentComplete",eventCat:"Video Engagement",eventAct:digitalData.page.videoname,eventLbl:"100% Completion",eventVal:0,nonInteraction:0,videoName:digitalData.page.videoname,videoAction:"100% Completion",videoPercentageWatched:"100%",videoSecondsWatched:digitalData.page.videoduration,videoCompletions:1}
}).register("leftnavigationclicks",function(){return{event:"custEv_leftNavLinkClick",eventCat:"Navigation Link Click",eventAct:"Left",eventLbl:b(),eventVal:0,nonInteraction:0}
}).register("intsearch",function(){return{event:"custEv_siteSearch",eventCat:"Site Search",eventAct:digitalData.page.searchterm,eventLbl:digitalData.page.numsearchresults,eventVal:0,nonInteraction:0}
...
}).register("autoformcomplete",function(){return{event:"custEv_autoFormComplete",eventCat:"Form Engagement",eventAct:"Auto Form Completion",eventLbl:Page.getFormType(),formJoinIDUser:digitalData.user[0].profile[0].transactionid,formJoinIDHit:digitalData.user[0].profile[0].transactionid,formID:digitalData.page.offerid,internalDrivers:digitalData.page.internaldrivers,autoFormCompletions:1,nonInteraction:0}
}).register("blogtagimpression",function(){return{event:"custEv_blogTagImpression",eventCat:"Blog Tag Impression",eventAct:digitalData.page.blogArticleTitle,eventLbl:digitalData.page.blogpostmeta,eventVal:0,nonInteraction:0,blogAuthor:digitalData.page.authorName,blogTag:digitalData.page.blogpostmeta,blogTagImpressions:1}
});
function b(){var g=digitalData.page.moduletracking.split("|")[1];
var c=Page.getNameNoLocale();
var d=c.replace(Page.getServer()+":","");
return d
}}());
(function(){var a=(function(){return Object.keys(digitalData.util.event).reduce(function(c,b){return c+digitalData.util.event[b]+" "
},"")
}());
...
var g=digitalData.util.mappings.google.getEvent(c);
if(typeof g!=="undefined"){dataLayer.push(g)
}}}})
}());