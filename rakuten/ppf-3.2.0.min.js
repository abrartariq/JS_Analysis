...
;a.code&&(c+=" ["+a.code+"]"),c+=" on "+d.options.global.target,a.message&&(c+=" ("+a.message+")"),b={fileUrl:x.FILE_URL,type:x.API_ERROR_TYPE,msg:c},r(d.shisa,b)}).done(function(a){s=a}).always(function(){f(d)})})}function f(b){var c=s&&s.getItems(b.options.global.targetId);if(!c||0===c.length){if(!b.options.global.defaultContent)return;c=b.options.global.defaultContent}b.items=c.slice(0,b.options.global.maxItems||c.length),
